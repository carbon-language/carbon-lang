...
bool type_equal(Value* t1, Value* t2) {
  return (t1->tag == VarTV && t2->tag == VarTV &&
         ...
         (t1->tag == TupleTV && t2->tag == TupleTV &&
          fields_equal(t1->u.tuple_type.fields, t2->u.tuple_type.fields));
}

void expect_type(int lineno, string context, Value* expected, Value* actual) {
...
void print_error_string(string s) { cerr << s; }
...
TCResult typecheck_exp(
  ...
  switch (e->tag) {
    case PatternVariable: {
      ...
      auto t =
          to_type(e->lineno, interp_exp(ct_env, e->u.pattern_variable.type));
      if (t->tag == AutoTV) {
        if (expected == 0) {
    ...
    case Index: {
      auto res =
          typecheck_exp(e->u.get_field.aggregate, env, ct_env, 0, ValueContext);
      auto t = res.type;
      switch (t->tag) {
        case TupleTV: {
    ...
    case Tuple: {
      ...
      for (auto arg = e->u.tuple.fields->begin();
        ...
        if (expected && expected->tag == TupleTV) {
          try {
            arg_expected =
                find_alist(arg->first, expected->u.tuple_type.fields);
          } catch (std::domain_error de) {
        ...
        auto arg_res =
            typecheck_exp(arg->second, new_env, ct_env, arg_expected, context);
        new_env = arg_res.env;
    ...
    case Call: {
      auto fun_res =
          typecheck_exp(e->u.call.function, env, ct_env, 0, ValueContext);
...
pair<Expression*, Statement*> typecheck_case(Value* expected, Expression* pat,
                                             Statement* body, TypeEnv* env,
                                             Env* ct_env, Value* ret_type) {
...
TCStatement typecheck_stmt(Statement* s, TypeEnv* env, Env* ct_env,
                           Value* ret_type) {
  if (!s) {
  ...
  switch (s->tag) {
    case Match: {
      auto res =
          typecheck_exp(s->u.match_stmt.exp, env, ct_env, 0, ValueContext);
    ...
    case Sequence: {
      auto stmt_res = typecheck_stmt(s->u.sequence.stmt, env, ct_env, ret_type);
    ...
    case Assign: {
      auto rhs_res =
          typecheck_exp(s->u.assign.rhs, env, ct_env, 0, ValueContext);
...
struct FunctionDefinition* typecheck_fun_def(struct FunctionDefinition* f,
...
Value* type_of_fun_def(TypeEnv* env, Env* ct_env,
  ...
  auto param_res =
      typecheck_exp(fun_def->param_pattern, env, ct_env, 0, PatternContext);
...
Value* type_of_struct_def(struct StructDefinition* sd, TypeEnv* env,
  ...
  for (auto m = sd->members->begin(); m != sd->members->end(); ++m) {
    if ((*m)->tag == FieldMember) {
      auto t = to_type(sd->lineno, interp_exp(ct_top, (*m)->u.field.type));
