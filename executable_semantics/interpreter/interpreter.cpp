...
auto Interpreter::EvalPrim(Operator op, const std::vector<const Value*>& args,
...
void Interpreter::InitGlobals(const std::vector<const Declaration*>& fs) {
  for (const auto d : fs) {
    InitEnv(*d, &globals);
  }
}
...
void Interpreter::DeallocateLocals(Frame* frame) {
...
auto Interpreter::CreateTuple(Action* act, const Expression* exp)
    -> const Value* {
  //    { { (v1,...,vn) :: C, E, F} :: S, H}
  // -> { { `(v1,...,vn) :: C, E, F} :: S, H}
      ...
      switch (val->Tag()) {
        case Value::Kind::TupleValue: {
          ...
          for (const TupleElement& pattern_element : pat_tup.Elements()) {
            std::optional<const Value*> value_field =
...
auto Interpreter::StepLvalue() -> Transition {
  ...
  switch (exp->Tag()) {
    case Expression::Kind::IdentifierExpression: {
      ...
      const Value* v = arena->New<PointerValue>(pointer);
      return Done{v};
    }
  ...
  switch (exp->Tag()) {
    case Expression::Kind::IndexExpression: {
      if (act->Pos() == 0) {
        ...
        std::string f =
            std::to_string(cast<IntValue>(*act->Results()[1]).Val());
        std::optional<const Value*> field = tuple->FindField(f);
        if (!field) {
          FATAL_RUNTIME_ERROR_NO_LINE()
...
static auto IsWhileAct(Action* act) -> bool {
  switch (act->Tag()) {
...
static auto HasLocalScope(Action* act) -> bool {
  switch (act->Tag()) {
    case Action::Kind::StatementAction:
      switch (cast<StatementAction>(*act).Stmt()->Tag()) {
...
auto Interpreter::StepStmt() -> Transition {
  ...
  switch (stmt->Tag()) {
    ...
    case Statement::Kind::Continuation: {
      ...
      auto scopes = Stack<Scope*>(arena->New<Scope>(CurrentEnv()));
      Stack<Action*> todo;
...
class Interpreter::DoTransition {
 public:
  ...
  void operator()(const RunAgain&) {
    Action* action = interpreter->stack.Top()->todo.Top();
    action->SetPos(action->Pos() + 1);
  }
  ...
  void operator()(const CallFunction& call) {
    ...
    auto scopes = Stack<Scope*>(interpreter->arena->New<Scope>(values, params));
    CHECK(call.function->Body()) << "Calling a function that's missing a body";
    auto todo = Stack<Action*>(
...
auto Interpreter::InterpProgram(const std::vector<const Declaration*>& fs)
    -> int {
  ...
  const Expression* arg = arena->New<TupleLiteral>(loc);
  const Expression* call_main = arena->New<CallExpression>(
      loc, arena->New<IdentifierExpression>(loc, "main"), arg);
  auto todo = Stack<Action*>(arena->New<ExpressionAction>(call_main));
  auto scopes = Stack<Scope*>(arena->New<Scope>(globals));
  stack = Stack<Frame*>(arena->New<Frame>("top", scopes, todo));
...
auto Interpreter::InterpExp(Env values, const Expression* e) -> const Value* {
  CHECK(program_value == std::nullopt);
  auto program_value_guard =
      llvm::make_scope_exit([&] { program_value = std::nullopt; });
  auto todo = Stack<Action*>(arena->New<ExpressionAction>(e));
  auto scopes = Stack<Scope*>(arena->New<Scope>(values));
  stack = Stack<Frame*>(arena->New<Frame>("InterpExp", scopes, todo));
...
auto Interpreter::InterpPattern(Env values, const Pattern* p) -> const Value* {
  CHECK(program_value == std::nullopt);
  auto program_value_guard =
      llvm::make_scope_exit([&] { program_value = std::nullopt; });
  auto todo = Stack<Action*>(arena->New<PatternAction>(p));
  auto scopes = Stack<Scope*>(arena->New<Scope>(values));
  stack = Stack<Frame*>(arena->New<Frame>("InterpPattern", scopes, todo));
