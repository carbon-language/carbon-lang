...
auto TypeChecker::ReifyType(const Value* t, SourceLocation loc)
    -> const Expression* {
  switch (t->Tag()) {
...
auto TypeChecker::Substitute(TypeEnv dict, const Value* type) -> const Value* {
  switch (type->Tag()) {
    case Value::Kind::VariableType: {
...
auto TypeChecker::TypeCheckExp(const Expression* e, TypeEnv types, Env values)
    -> TCExpression {
  if (tracing_output) {
    ...
    case Expression::Kind::IndexExpression: {
      ...
      switch (t->Tag()) {
        case Value::Kind::TupleValue: {
          ...
          std::optional<const Value*> field_t =
        ...
        case Value::Kind::ClassType: {
          ...
          for (auto& field : t_class.Fields()) {
            if (access.Field() == field.first) {
              const Expression* new_e = arena->New<FieldAccessExpression>(
...
auto TypeChecker::TypeCheckPattern(const Pattern* p, TypeEnv types, Env values,
                                   std::optional<const Value*> expected)
    -> TCPattern {
  if (tracing_output) {
  ...
  switch (p->Tag()) {
    ...
    case Pattern::Kind::TuplePattern: {
      ...
      for (size_t i = 0; i < tuple.Fields().size(); ++i) {
        ...
        std::optional<const Value*> expected_field_type;
        if (expected) {
    ...
    case Pattern::Kind::AlternativePattern: {
      ...
      std::optional<const Value*> parameter_types =
...
auto TypeChecker::TypeCheckCase(const Value* expected, const Pattern* pat,
...
auto TypeChecker::TypeCheckStmt(const Statement* s, TypeEnv types, Env values,
  ...
  switch (s->Tag()) {
    case Statement::Kind::Match: {
      ...
      std::vector<std::pair<const Pattern*, const Statement*>> new_clauses;
    ...
    case Statement::Kind::Sequence: {
      ...
      std::optional<const Statement*> next_stmt;
      if (seq.Next()) {
    ...
    case Statement::Kind::If: {
      ...
      auto then_res = TypeCheckStmt(if_stmt.ThenStmt(), types, values, ret_type,
                                    is_omitted_ret_type);
      std::optional<const Statement*> else_stmt;
      if (if_stmt.ElseStmt()) {
...
auto TypeChecker::CheckOrEnsureReturn(std::optional<const Statement*> opt_stmt,
...
auto TypeChecker::TypeCheckFunDef(const FunctionDefinition* f, TypeEnv types,
                                  Env values) -> const FunctionDefinition* {
  ...
  std::optional<const Statement*> body_stmt;
  if (f->body) {
...
auto TypeChecker::MakeTypeChecked(const Declaration* d, const TypeEnv& types,
...
auto TypeChecker::TopLevel(const std::vector<const Declaration*>& fs)
    -> TypeCheckContext {
