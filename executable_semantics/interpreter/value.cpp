...
auto GetMember(Arena* arena, const Value* v, const std::string& f,
               SourceLocation loc) -> const Value* {
  switch (v->Tag()) {
    case Value::Kind::StructValue: {
      std::optional<const Value*> field =
    ...
    case Value::Kind::TupleValue: {
      std::optional<const Value*> field = cast<TupleValue>(*v).FindField(f);
      if (!field) {
...
auto SetFieldImpl(Arena* arena, const Value* value,
...
auto Value::SetField(Arena* arena, const FieldPath& path,
                     const Value* field_value, SourceLocation loc) const
    -> const Value* {
  return SetFieldImpl(arena, this, path.components.begin(),
...
void Value::Print(llvm::raw_ostream& out) const {
  switch (Tag()) {
    ...
    case Value::Kind::PointerValue:
      out << "" << cast<PointerValue>(*this).Val() << ">";
      break;
    ...
    case Value::Kind::ContinuationValue: {
      out << "{";
      llvm::ListSeparator sep(" :: ");
      for (Frame* frame : cast<ContinuationValue>(*this).Stack()) {
        out << sep << *frame;
      }
      out << "}";
      break;
...
auto ValueEqual(const Value* v1, const Value* v2, SourceLocation loc) -> bool {
  ...
  switch (v1->Tag()) {
    ...
    case Value::Kind::FunctionValue: {
      std::optional<const Statement*> body1 = cast<FunctionValue>(*v1).Body();
      std::optional<const Statement*> body2 = cast<FunctionValue>(*v2).Body();
