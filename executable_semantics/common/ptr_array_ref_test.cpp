...
TEST(PtrArrayRefTest, MutableT) {
  ...
  static_assert(IsConstdecltype(ref[0].Get())*);
}
...
TEST(PtrArrayRefTest, Iterator) {
  Arena arena;
  std::vector<int*> v = {arena.New<int>(0), arena.New<int>(1)};
  PtrArrayRef<int> ref(v);

  int i = 0;
  for (const int* ptr : ref) {
    EXPECT_EQ(*ptr, i);
    ++i;
  }

  for (const int* ptr : llvm::reverse(ref)) {
    --i;
    EXPECT_EQ(*ptr, i);
  }
}
