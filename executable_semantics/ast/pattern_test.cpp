...
TEST(PatternTest, EmptyAsPattern) {
  ...
  const Pattern* pattern = PatternFromParenContents(/*loc=*/1, contents);
  EXPECT_EQ(pattern->Loc(), 1);
...
TEST(PatternTest, EmptyAsTuplePattern) {
  ...
  const TuplePattern* tuple =
      TuplePatternFromParenContents(/*loc=*/1, contents);
  EXPECT_EQ(tuple->Loc(), 1);
...
TEST(PatternTest, UnaryNoCommaAsPattern) {
  ...
  ParenContents<Pattern> contents = {
      .elements = {{.name = std::nullopt,
                    .term = global_arena->RawNew<AutoPattern>(/*loc=*/2)}},
      .has_trailing_comma = false};

  const Pattern* pattern = PatternFromParenContents(/*loc=*/1, contents);
  EXPECT_EQ(pattern->Loc(), 2);
...
TEST(PatternTest, BinaryAsPattern) {
  ParenContents<Pattern> contents = {
      .elements = {{.name = std::nullopt,
                    .term = global_arena->RawNew<AutoPattern>(/*loc=*/2)},
                   {.name = std::nullopt,
                    .term = global_arena->RawNew<AutoPattern>(/*loc=*/3)}},
      .has_trailing_comma = true};
