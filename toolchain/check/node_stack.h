...
class NodeStack {
 public:
  ...
  auto Push(Parse::Lamp parse_lamp) -> void {
    CARBON_CHECK(ParseLampKindToIdKind(parse_tree_->node_kind(parse_lamp)) ==
                 IdKind::SoloParseLamp)
  ...
  auto PeekIs() const -> bool {
    return parse_tree_->node_kind(PeekParseLamp()) == RequiredParseKind;
  }
  ...
  template <Parse::LampKind::RawEnumType RequiredParseKind>
  auto PopForSoloParseLamp() -> Parse::Lamp {
  ...
  template <Parse::LampKind::RawEnumType RequiredParseKind>
  auto PopForSoloParseLampIf() -> std::optional<Parse::Lamp> {
    if (PeekIs<RequiredParseKind>()) {
      return PopForSoloParseLamp<RequiredParseKind>();
    }
    return std::nullopt;
  }
  ...
  auto PopExpression() -> SemIR::InstId {
    return PopExpressionWithParseLamp().second;
  }

  // Pops the top of the stack and returns the ID.
  template <Parse::LampKind::RawEnumType RequiredParseKind>
  auto Pop() -> auto {
    return PopWithParseLamp<RequiredParseKind>().second;
  }
 ...
 private:
  ...
  static constexpr auto ParseLampKindToIdKind(Parse::LampKind kind) -> IdKind {
    switch (kind) {
  ...
  // Pops the top of the stack and returns the parse_lamp and the ID.
  template <typename IdT>
  auto PopWithParseLamp() -> std::pair<Parse::Lamp, IdT> {
  ...
  auto RequireIdKind(Parse::LampKind parse_kind, IdKind id_kind) const -> void {
    CARBON_CHECK(ParseLampKindToIdKind(parse_kind) == id_kind)
