...
class NodeStack {
 public:
  ...
  auto Push(Parse::Lamp parse_node) -> void {
  ...
  auto Push(Parse::Lamp parse_node, IdT id) -> void {
    CARBON_CHECK(ParseNodeKindToIdKind(parse_tree_->node_kind(parse_node)) ==
                 IdTypeToIdKind<IdT>())
  ...
  // top node.
  auto PeekParseNode() const -> Parse::Lamp { return stack_.back().parse_node; }
 ...
 private:
  ...
  // An entry in stack_.
  struct Entry {
    explicit Entry(Parse::Lamp parse_node, SemIR::InstId inst_id)
        : parse_node(parse_node), inst_id(inst_id) {}
    explicit Entry(Parse::Lamp parse_node, SemIR::InstBlockId inst_block_id)
        : parse_node(parse_node), inst_block_id(inst_block_id) {}
    explicit Entry(Parse::Lamp parse_node, SemIR::FunctionId function_id)
        : parse_node(parse_node), function_id(function_id) {}
    explicit Entry(Parse::Lamp parse_node, SemIR::ClassId class_id)
        : parse_node(parse_node), class_id(class_id) {}
    explicit Entry(Parse::Lamp parse_node, StringId name_id)
        : parse_node(parse_node), name_id(name_id) {}
    explicit Entry(Parse::Lamp parse_node, SemIR::TypeId type_id)
  ...
  template <Parse::LampKind::RawEnumType RequiredParseKind>
  auto RequireParseKind(Parse::Lamp parse_node) const -> void {
