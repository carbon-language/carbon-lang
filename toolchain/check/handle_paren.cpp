...
auto HandleParenExpression(Context& context, Parse::Lamp parse_lamp) -> bool {
...
auto HandleTupleLiteralComma(Context& context, Parse::Lamp /*parse_lamp*/)
    -> bool {
  context.ParamOrArgComma();
  return true;
}

auto HandleTupleLiteral(Context& context, Parse::Lamp parse_lamp) -> bool {
  ...
  auto value_id =
      context.AddInst(SemIR::TupleLiteral{parse_lamp, type_id, refs_id});
  context.lamp_stack().Push(parse_lamp, value_id);
  return true;
}
