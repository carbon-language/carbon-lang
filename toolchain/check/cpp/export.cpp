...
struct FunctionInfo {
  struct Param {
    Param(Context& context, SemIR::InstId param_inst_id,
          ...
          const SemIR::File* default_value_sem_ir = nullptr)
        : pattern_inst_id(param_inst_id),
          ...
          default_value_sem_ir(default_value_sem_ir) {}
    ...
    // The file container in which the default value instruction is stored,
    // or nullptr if no default was specified.
    const SemIR::File* default_value_sem_ir;
  };
  ...
  explicit FunctionInfo(Context& context, SemIR::FunctionId function_id,
    ...
    for (auto i = explicit_begin; i != ranges.explicit_end().index; ++i) {
      // Look up overall pattern to see if it has a default value.
      auto [pattern_file, pattern_inst_id] = SemIR::GetCanonicalFileAndInstId(
          &context.sem_ir(), function_patterns[i]);
      if (auto default_value_inst =
              pattern_file->insts().TryGetAs<SemIR::DefaultValuePattern>(
                  pattern_inst_id)) {
        const auto& default_value = pattern_file->default_values().Get(
            default_value_inst->default_value_id);
        explicit_params.push_back(Param(context, function_params[i],
                                        default_value.value_id, pattern_file));
      } else {
        explicit_params.push_back(Param(context, function_params[i]));
      }
    }
...
static auto BuildCppFunctionDeclForNonGenericCarbonFn(Context& context,
  ...
  if (target.self_param) {
    ...
    default_arguments.push_back(nullptr);
  }
  ...
  CARBON_CHECK(default_arguments.size() == cpp_param_types.size());
...
static auto BuildCppToCarbonThunkDecl(Context& context, SemIR::LocId loc_id,
  ...
  for (auto [i, type] : llvm::enumerate(thunk_function_type->param_types())) {
    ...
    clang::ParmVarDecl* thunk_param = clang::ParmVarDecl::Create(
        ...
        /*TInfo=*/nullptr, clang::SC_None, default_argument);
