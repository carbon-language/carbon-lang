...
class SubstInstCallbacks {
 public:
  ...
  enum SubstResult {
    ...
    SubstAgain,
    // Attempt to substitute into the operands of the instruction. If the
    // operands change, then the rebuilt instruction will be given to `Subst`
    // again afterward. This allows for the uncommon case of substituting from
    // the inside out.
    SubstOperandsAndAgain,
  };
  ...
  // SubstOperandsAndAgain to recurse on the instructions operands and then
  // substitute the resulting instruction afterward, if the operands changed.
  //
  // When SubstOperands, SubstAgain, or SubstOperandsAndAgain is returned, it
