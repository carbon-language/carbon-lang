  ...
  switch (builtin_kind) {
    ...
    case SemIR::BuiltinFunctionKind::IntMakeTypeUnsigned: {
      return MakeConstantResult(
          ...
          SemIR::IntType{
              .type_id = context.GetBuiltinType(SemIR::BuiltinKind::TypeType),
              .int_kind = SemIR::IntKind::Unsigned,
              .bit_width_id = arg_ids[0]},
          phase);
    }
    ...
    case SemIR::IntType::Kind:
      return RebuildAndValidateIfFieldsAreConstant(
          ...
          [&](SemIR::IntType result) {
            auto bit_width = context.insts().TryGetAs<SemIR::IntLiteral>(
                result.bit_width_id);
            if (!bit_width) {
              // Symbolic bit width.
              return true;
            ...
            if (bit_width_val.isZero() ||
              ...
              context.emitter().Emit(
                  inst.As<SemIR::IntType>().bit_width_id, IntWidthNotPositive,
            ...
            if (bit_width_val.ugt(MaxIntWidth)) {
              CARBON_DIAGNOSTIC(IntWidthTooLarge, Error,
                                ...
                                TypedInt, int);
              context.emitter().Emit(
                  inst.As<SemIR::IntType>().bit_width_id, IntWidthTooLarge,
                  TypedInt{bit_width->type_id, bit_width_val}, MaxIntWidth);
              return false;
            }
            return true;
          },
