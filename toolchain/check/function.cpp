...
struct BuiltinFunctionSignatureInsts {
  SemIR::InstBlockId implicit_param_patterns_id = SemIR::InstBlockId::None;
  SemIR::InstBlockId param_patterns_id = SemIR::InstBlockId::Empty;
...
static auto MakeBuiltinFunctionSignatureInsts(
    Context& context, SemIR::LocId loc_id, BuiltinFunctionSignature signature)
    -> BuiltinFunctionSignatureInsts {
  BuiltinFunctionSignatureInsts sig_insts;
  ...
  if (signature.self_type_id.has_value()) {
    ...
    sig_insts.self_param_id = AddParamPattern(
        context, loc_id, SemIR::NameId::SelfValue, self_type_region_id,
        signature.self_type_id, signature.self_is_ref);
    sig_insts.implicit_param_patterns_id =
        context.inst_blocks().Add({sig_insts.self_param_id});
  ...
  if (!signature.param_type_ids.empty()) {
    ...
    sig_insts.param_patterns_id = context.inst_block_stack().Pop();
  }

  if (signature.return_type_id.has_value()) {
    // Build and add the return type. We always use an initializing form for
    // now.
    auto return_form = ExprAsReturnForm(
        context, loc_id, context.types().GetInstId(signature.return_type_id));
    sig_insts.return_type_inst_id = return_form.type_component_id;
    sig_insts.return_form_inst_id = return_form.form_inst_id;
    sig_insts.return_patterns_id =
        AddReturnPatterns(context, loc_id, return_form);
  }

  auto [call_param_patterns_id, call_params_id] = CalleePatternMatch(
      context, sig_insts.implicit_param_patterns_id,
      sig_insts.param_patterns_id, sig_insts.return_patterns_id);
  sig_insts.call_param_patterns_id = call_param_patterns_id;
  sig_insts.call_params_id = call_params_id;
...
auto MakeBuiltinFunction(Context& context, SemIR::LocId loc_id,
  ...
  auto sig_insts =
      MakeBuiltinFunctionSignatureInsts(context, loc_id, signature);
      ...
      SemIR::Function{
          {
              ...
              .implicit_param_patterns_id =
                  sig_insts.implicit_param_patterns_id,
              .param_patterns_id = sig_insts.param_patterns_id,
              .is_extern = false,
          ...
          {
              .call_param_patterns_id = sig_insts.call_param_patterns_id,
              .call_params_id = sig_insts.call_params_id,
              .return_type_inst_id = sig_insts.return_type_inst_id,
              .return_form_inst_id = sig_insts.return_form_inst_id,
              .return_patterns_id = sig_insts.return_patterns_id,
              .self_param_id = sig_insts.self_param_id,
          }});
