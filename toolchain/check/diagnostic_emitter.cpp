...
auto DiagnosticEmitter::ConvertArg(llvm::Any arg) const -> llvm::Any {
  ...
  if (auto* expr = llvm::any_cast<InstIdAsConst>(&arg)) {
    return "`" + StringifyConst(*sem_ir_, expr->inst_id) + "`";
  }
  if (auto* type_expr = llvm::any_cast<InstIdAsType>(&arg)) {
    return "`" + StringifyConst(*sem_ir_, type_expr->inst_id) + "`";
  }
  if (auto* type_expr = llvm::any_cast<InstIdAsRawType>(&arg)) {
    return StringifyConst(*sem_ir_, type_expr->inst_id);
  }
  if (auto* type = llvm::any_cast<TypeIdAsRawType>(&arg)) {
    return StringifyConst(*sem_ir_, sem_ir_->types().GetInstId(type->type_id));
  }
  if (auto* type_id = llvm::any_cast<SemIR::TypeId>(&arg)) {
    return "`" +
           StringifyConst(*sem_ir_, sem_ir_->types().GetInstId(*type_id)) + "`";
  }
