...
auto GetHighestAllowedAccess(Context& context, SemIR::LocId loc_id,
  ...
  if (auto class_type = context.constant_values().TryGetAs<SemIR::ClassType>(
          name_scope_const_id)) {
...
auto GetAssociatedValue(Context& context, SemIR::LocId loc_id,
  ...
  auto assoc_entity = context.constant_values().GetAs<SemIR::AssociatedEntity>(
...
auto PerformCompoundMemberAccess(
  ...
  if (member_id == member_expr_id &&
    ...
    if (context.constant_values().Is<SemIR::TupleType>(base_type_const_id)) {
...
auto PerformTupleAccess(Context& context, SemIR::LocId loc_id,
  ...
  auto index_literal =
      context.constant_values().GetAs<SemIR::IntValue>(index_const_id);
