...
auto ConvertCallArgs(Context& context, SemIR::LocId call_loc_id,
  ...
  for (auto implicit_param_id : implicit_param_patterns) {
    auto param_pattern =
        context.insts().GetAs<SemIR::ParamPattern>(implicit_param_id);
    auto implicit_param = context.insts().Get(param_pattern.subpattern_id);
    ...
    if (SemIR::Function::GetNameFromParamPatternId(
      ...
      CARBON_CHECK(!param_pattern.runtime_index.is_valid(),
  ...
  for (auto [i, arg_id, param_pattern_id] :
    ...
    auto param_type_id = SemIR::GetTypeInSpecific(
        context.sem_ir(), callee_specific_id, param_pattern.type_id);
    ...
    CARBON_CHECK(
        static_cast<int32_t>(args.size()) == param_pattern.runtime_index.index,
