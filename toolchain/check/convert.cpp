...
static auto PerformBuiltinConversion(Context& context, Parse::Node parse_node,
  ...
  if (value_type_id == target.type_id) {
    auto value_cat = SemIR::GetExpressionCategory(semantics_ir, value_id);
    ...
    if (value_cat == SemIR::ExpressionCategory::Initializing &&
        ...
        SemIR::GetInitializingRepresentation(semantics_ir, value_type_id)
                .kind == SemIR::InitializingRepresentation::ByCopy) {
      auto value_rep =
          SemIR::GetValueRepresentation(semantics_ir, value_type_id);
