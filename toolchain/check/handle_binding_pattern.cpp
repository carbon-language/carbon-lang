    ...
    case FullPatternStack::Kind::ExplicitParamList: {
      ...
      switch (node_kind) {
        ...
        case Parse::NodeKind::FormBindingPattern: {
          ...
          if (is_ref) {
            param_pattern_id = AddPatternInst(
                context,
                SemIR::LocIdAndInst::RuntimeVerified(
                    context.sem_ir(), node_id,
                    SemIR::RefParamPattern{.type_id = pattern_type_id,
                                           .pretty_name_id = name_id}));
          } else if (node_kind == Parse::NodeKind::FormBindingPattern) {
            param_pattern_id = AddPatternInst(
                context, SemIR::LocIdAndInst::RuntimeVerified(
                             context.sem_ir(), node_id,
                             SemIR::FormParamPattern{.type_id = pattern_type_id,
                                                     .pretty_name_id = name_id,
                                                     .form_id = form_id}));
          } else {
            param_pattern_id = AddPatternInst(
                ...
                SemIR::LocIdAndInst::RuntimeVerified(
                    context.sem_ir(), node_id,
                    SemIR::ValueParamPattern{.type_id = pattern_type_id,
                                             .pretty_name_id = name_id}));
          }
