...
-   A name binding can be used within the same pattern that declares it:
    ...
    handle its declaration, and it needs to resolve to the `BindValueName`
    instruction that binds a value to that name. This means that the
    `BindValueName` instruction needs to be allocated during the pattern step,
    even though it is part of matching, not part of the pattern.
    `Context::bind_name_map` stores these `BindValueName`s, keyed by the
...
`BindValueName` instruction that binds the parameter name to the `ValueParam`
