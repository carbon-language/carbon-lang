...
class Formatter::FormatterImpl {
 public:
  explicit FormatterImpl(const Lex::TokenizedBuffer& tokenized_buffer,
  ...
  auto Print(llvm::raw_ostream& out) -> void {
    llvm::SaveAndRestore store_out(out_, &out);
    Format();
  }
  ...
  auto PrintPartialTrailingCodeBlock(llvm::ArrayRef<SemIR::InstId> block,
      ...
      -> void {
    llvm::SaveAndRestore store_indent(indent_, indent);
    llvm::SaveAndRestore store_out(out_, &out);

    *out_ << ' ';
    OpenBrace();
    constexpr int MaxPrinted = 10;
    if (block.size() > MaxPrinted) {
      Indent();
      *out_ << "... skipping " << (block.size() - MaxPrinted) << " insts ...\n";
      block = block.take_back(MaxPrinted);
    }
