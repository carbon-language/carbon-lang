...
struct NameId : public IdBase, public Printable<NameId> {
  ...
  // The number of special names that exist, and will need storage in name
  // lookup.
  static const int SpecialValueCount;
...
constexpr int NameId::SpecialValueCount = 5;
// Enforce the link between SpecialValueCount and the last special value.
static_assert(NameId::SpecialValueCount ==
              NameId::InvalidIndex - NameId::Base.index);
