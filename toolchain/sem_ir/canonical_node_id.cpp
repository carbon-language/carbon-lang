...
static auto FollowImportRef(
    llvm::SmallVector<CanonicalNodeId>& canonical_node_ids,
  ...
  if (import_loc_id.is_node_id()) {
    ...
    canonical_node_ids.push_back({.check_ir_id = cursor_ir->check_ir_id(),
...
static auto HandleLocId(llvm::SmallVector<CanonicalNodeId>& canonical_node_ids,
                        const File*& cursor_ir, InstId& cursor_inst_id,
                        LocId loc_id) -> bool {
  if (loc_id.is_import_ir_inst_id()) {
    FollowImportRef(canonical_node_ids, cursor_ir, cursor_inst_id,
...
static auto GetCanonicalNodeIdImpl(
    llvm::SmallVector<CanonicalNodeId>& canonical_node_ids,
  ...
  while (true) {
    if (cursor_inst_id.has_value()) {
      ...
      if (auto loc_id = cursor_ir->insts().GetLocId(cursor_inst_id);
          loc_id.has_value()) {
        if (HandleLocId(canonical_node_ids, cursor_ir, cursor_inst_id,
                        loc_id)) {
          return;
        }
        continue;
...
auto GetCanonicalNodeId(const File* sem_ir, InstId inst_id)
  ...
  llvm::SmallVector<CanonicalNodeId> canonical_node_ids;
  GetCanonicalNodeIdImpl(canonical_node_ids, sem_ir, inst_id);
  return canonical_node_ids;
}
...
auto GetCanonicalNodeId(const File* sem_ir, LocId loc_id)
  ...
  if (HandleLocId(canonical_node_ids, cursor_ir, cursor_inst_id, loc_id)) {
    return canonical_node_ids;
  }
  CARBON_CHECK(cursor_inst_id.has_value(), "Should be set by HandleLocId");
  GetCanonicalNodeIdImpl(canonical_node_ids, cursor_ir, cursor_inst_id);
  return canonical_node_ids;
}
