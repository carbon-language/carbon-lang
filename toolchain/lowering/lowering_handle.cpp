...
auto LoweringHandleTupleTypeField(LoweringFunctionContext&, SemanticsNodeId,
                                  SemanticsNode) -> void {
  // No action to take
}

auto LoweringHandleTupleMemberAccess(LoweringFunctionContext& context,
                                     SemanticsNodeId node_id,
                                     SemanticsNode node) -> void {
  auto [tuple_id, member_index] = node.GetAsTupleMemberAccess();
  auto tuple_type_id = context.semantics_ir().GetNode(tuple_id).type_id();
  auto* llvm_type = context.GetType(tuple_type_id);

  // Get type information for member names.
  auto type_refs = context.semantics_ir().GetNodeBlock(
      context.semantics_ir()
          .GetNode(context.semantics_ir().GetType(tuple_type_id))
          .GetAsTupleType());

  auto* gep = context.builder().CreateStructGEP(
      llvm_type, context.GetLocal(tuple_id), member_index.index);
  context.SetLocal(node_id, gep);
}
