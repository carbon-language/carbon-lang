...
TEST_F(TypedNodeTest, Choice) {
  ...
  auto ordering_choice = tree->ExtractAs<ChoiceDefinition>(file.decls[0]);
  ASSERT_TRUE(ordering_choice.has_value());
  auto ordering_sig = tree->Extract(ordering_choice->signature);
  ASSERT_TRUE(ordering_sig.has_value());
  ASSERT_FALSE(ordering_sig->parameters.has_value());

  auto ordering_name = tree->ExtractAs<IdentifierName>(ordering_sig->name);
  ASSERT_TRUE(ordering_name.has_value());

  ASSERT_EQ(ordering_choice->alternatives.size(), 4);
  for (size_t i = 0; i < ordering_choice->alternatives.size() - 1; ++i) {
    const auto& alternative_item = ordering_choice->alternatives[i];
    ASSERT_TRUE(alternative_item.comma.has_value()) << "alternative " << i;
    ASSERT_TRUE(tree->ExtractAs<IdentifierName>(alternative_item.value.name))
        << "alternative " << i;
    ASSERT_FALSE(alternative_item.value.parameters.has_value())
        << "alternative " << i;
  }

  const auto& last_alternative = ordering_choice->alternatives[3];
  ASSERT_FALSE(last_alternative.comma.has_value());
  ASSERT_TRUE(tree->ExtractAs<IdentifierName>(last_alternative.value.name));
  ASSERT_FALSE(last_alternative.value.parameters.has_value());
}

TEST_F(TypedNodeTest, ChoiceParameters) {
  auto* tree = &GetTree(R"carbon(
  ...
  auto optional_element_choice =
      tree->ExtractAs<ChoiceDefinition>(file.decls[0]);
  ASSERT_TRUE(optional_element_choice.has_value());
  auto optional_element_sig = tree->Extract(optional_element_choice->signature);
  ASSERT_TRUE(optional_element_sig.has_value());
  ASSERT_TRUE(optional_element_sig->parameters.has_value());
  ASSERT_TRUE(tree->ExtractAs<TuplePattern>(*optional_element_sig->parameters)
                  .has_value());
}

TEST_F(TypedNodeTest, ChoiceAlternativeParameters) {
  auto* tree = &GetTree(R"carbon(
    choice OptionalElement(T:! type) {
      Element(value: T)
    }
  ...
  auto optional_element_choice =
      tree->ExtractAs<ChoiceDefinition>(file.decls[0]);
  ASSERT_TRUE(optional_element_choice.has_value());
  ASSERT_EQ(optional_element_choice->alternatives.size(), 1);

  const auto& alternative = optional_element_choice->alternatives[0];
  ASSERT_TRUE(alternative.value.parameters.has_value());
  ASSERT_TRUE(
      tree->ExtractAs<TuplePattern>(*alternative.value.parameters).has_value());
}
