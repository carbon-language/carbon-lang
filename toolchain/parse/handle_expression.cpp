...
auto HandleExpressionLoop(Context& context) -> void {
  ...
  if (is_binary) {
    if (operator_kind == Lex::TokenKind::And ||
      ...
      context.AddInst(NodeKind::ShortCircuitOperand, state.token,
    ...
    context.AddInst(NodeKind::PostfixOperator, state.token, state.subtree_start,
                    state.has_error);
...
auto HandleExpressionLoopForBinary(Context& context) -> void {
  ...
  context.AddInst(NodeKind::InfixOperator, state.token, state.subtree_start,
                  state.has_error);
...
auto HandleExpressionLoopForPrefix(Context& context) -> void {
  ...
  context.AddInst(NodeKind::PrefixOperator, state.token, state.subtree_start,
                  state.has_error);
  state.state = State::ExpressionLoop;
  state.has_error = false;
  context.PushState(state);
...
auto HandleIfExpressionFinishCondition(Context& context) -> void {
  ...
  context.AddInst(NodeKind::IfExpressionIf, state.token, state.subtree_start,
                  state.has_error);
...
auto HandleIfExpressionFinishThen(Context& context) -> void {
  ...
  context.AddInst(NodeKind::IfExpressionThen, state.token, state.subtree_start,
                  state.has_error);
...
auto HandleIfExpressionFinish(Context& context) -> void {
  ...
  context.AddInst(NodeKind::IfExpressionElse, state.token, state.subtree_start,
                  state.has_error);
}
...
auto HandleExpressionStatementFinish(Context& context) -> void {
  ...
  if (auto semi = context.ConsumeIf(Lex::TokenKind::Semi)) {
    context.AddInst(NodeKind::ExpressionStatement, *semi, state.subtree_start,
                    state.has_error);
    return;
  }
  ...
  if (auto semi_token = context.SkipPastLikelyEnd(state.token)) {
    context.AddInst(NodeKind::ExpressionStatement, *semi_token,
                    state.subtree_start,
                    /*has_error=*/true);
    return;
  }
