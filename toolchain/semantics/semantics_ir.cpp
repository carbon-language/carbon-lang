...
void SemanticsIR::Block::Add(llvm::StringRef name, NamedEntity entity) {
  ordering_.push_back(entity);
  name_lookup_.insert({name, entity});
}
...
auto SemanticsIR::AddFunction(Block& block, ParseTree::Node decl_node,
  ...
  block.Add(parse_tree_->GetNodeText(name_node),
            NamedEntity(NamedEntity::Kind::Function, index));
