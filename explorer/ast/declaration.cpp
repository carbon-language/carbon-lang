...
void TypeVariableBinding::Print(llvm::raw_ostream& out) const {
  out << name() << ":! " << type();
}

void TypeVariableBinding::PrintID(llvm::raw_ostream& out) const {
  out << name();
}
...
auto FunctionDeclaration::Create(
    ...
    bool is_exported, std::vector<Nonnull<AstNode*>> deduced_params,
    Nonnull<TuplePattern*> param_pattern, ReturnTerm return_term,
