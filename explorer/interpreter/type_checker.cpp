...
static auto IsConcreteType(Nonnull<const Value*> value) -> bool {
  switch (value->kind()) {
    ...
    case Value::Kind::PointerType: {
      return IsConcreteType(&cast<PointerType>(*value).type());
    }
  }
}
  ...
  switch (e->kind()) {
    ...
    case ExpressionKind::PrimitiveOperatorExpression: {
      ...
      switch (op.op()) {
        ...
        case Operator::Ptr:
          RETURN_IF_ERROR(
              ExpectType(e->source_loc(), "*", arena_->New<TypeType>(), ts[0]));
...
auto TypeChecker::TypeCheckPattern(
  ...
  switch (p->kind()) {
      ...
      if (expected) {
        if (IsConcreteType(type)) {
          ...
          if (!PatternMatch(type, *expected, binding.type().source_loc(),
                            std::nullopt, generic_args, this->arena_)) {
            return FATAL_COMPILATION_ERROR(binding.type().source_loc())
                   << "Type pattern '" << *type
    ...
    case PatternKind::VarPattern: {
      ...
      RETURN_IF_ERROR(TypeCheckPattern(&var_pattern.pattern(), expected,
                                       impl_scope,
      ...
      ASSIGN_OR_RETURN(Nonnull<const Value*> pattern_value,
                       InterpPattern(&var_pattern, arena_, trace_));
    ...
    case PatternKind::AddrBindingPattern:
      ...
      RETURN_IF_ERROR(TypeCheckPattern(&addr_binding_pattern.binding(),
      ...
      ASSIGN_OR_RETURN(Nonnull<const Value*> pattern_value,
                       InterpPattern(&addr_binding_pattern, arena_, trace_));
