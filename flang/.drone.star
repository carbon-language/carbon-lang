        ...
        "steps": [
            {
                ...
                "commands": [
                    ...
                    'env CC=clang-8 CXX=clang++-8 CXXFLAGS="-stdlib=libc++" LDFLAGS="-fuse-ld=lld" cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=../install -DLLVM_TARGETS_TO_BUILD=host -DLLVM_ENABLE_PROJECTS="mlir" ../llvm',
                    "ninja",
                    "cd ../..",
...
def gcc(arch):
    return {
			...
			"apt-get update && apt-get install -y cmake ninja-build llvm-dev libz-dev git",
		        "git clone https://github.com/llvm/llvm-project",
		        "mkdir llvm-project/build && cd llvm-project/build",
		        'env CC=gcc CXX=g++ LDFLAGS="-fuse-ld=gold" cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=../install -DLLVM_TARGETS_TO_BUILD=host -DLLVM_ENABLE_PROJECTS="mlir" ../llvm',
                        "ninja",
                        "cd ../..",
                        "mkdir build && cd build",
