     ...
     &        openmp_version = 200805
      ...
      interface
          ...
          function omp_is_initial_device() bind(c)
              import
              integer (kind=omp_integer_kind) omp_is_initial_device
...
! Lock routines
     ...
     &    bind(c, name='omp_init_lock_')
              import
              integer (kind=omp_lock_kind), intent(out) :: lockvar
          end subroutine omp_init_lock

          subroutine omp_init_lock_with_hint(lockvar, hint)             &
     &    bind(c, name='omp_init_lock_with_hint_')
              import
              integer (kind=omp_lock_kind), intent(out) :: lockvar
     ...
     &    bind(c, name='omp_init_nest_lock_')
          ...
          subroutine omp_init_nest_lock_with_hint(lockvar, hint)        &
...
! Event Routine
          subroutine omp_fullfill_event(event) bind(c)
              import
