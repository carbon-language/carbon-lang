...
//
// Sizes and layouts of platform-specific Linux kernel data structures.
//===----------------------------------------------------------------------===//

// Only include headers under <linux/...> in this file.
// System includes belong to sanitizer_platform_limits_posix.cc.
...
// Missing from uapi kernel headers.
struct sockaddr {
...
// This one is not a kernel header, but we need it for SCSI ioctl constants.
#include <scsi/scsi.h>
#endif
...
namespace __sanitizer {
  ...
  // These two are also present on Mac, but defined in a different header.
#if !SANITIZER_ANDROID
