...
expect a human reader to recognize them. For example we want `a* - 4` to
consider the `*` unary and the `-` binary, while `a * -4` results in the reverse.
This largely requires whitespace on only one side of a unary operator and on
both sides of a binary operator. However, we'd also like to support binary
operators where a lack of whitespace reflects precedence such as
`2*x*x + 3*x + 1` where easy. The rules we use to achieve this are:
