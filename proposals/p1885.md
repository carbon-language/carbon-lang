...
-   [Future work](#future-work)
    ...
    -   [C++ interoperability](#c-interoperability)
        ...
        -   [Supporting Carbon `Iterate` in C++](#supporting-carbon-iterate-in-c)
...
#### Supporting Carbon `Iterate` in C++

Support for `Iterate` in C++ needs to provide `begin()` and `end()` methods or
free functions, in addition to `*` and `++` operators.

`Iterate` does not expose a way to have a cursor or iterator to the past-last
element. An option would be to have `end()` return a predefined invalid value,
to satisfy iterator comparison when `Next()` returns an empty `Optional`.

Below is an incomplete example of classes exposing those methods:

```cpp
template <typename T>
class Cursor {
  Cursor<T> FromCarbonCursor(...);
  Cursor<T> Invalid();

  T& operator*() const;
  Cursor<T>& operator++();
  // `true` if both are equal or invalid/empty
  bool operator==(const Cursor<T>& rhs);
  bool operator!=(const Cursor<T>& rhs);
  // [...]
};
...
class CppIterate {
  C begin() { return C::FromCarbonCursor(iterate_.NewCursor()); }
  C end() { return C::Invalid(); }
  // [...]
}
```
