...
A lazy iterator can also be returned using `yield`, giving directly control over
the values presented for each iteration.
...
The combined `Next` method forces us to return an `Optional`, which has a
downsides:

-   Potential performance overhead to wrapping and unwrapping, and storage
    overhead for `Optional` itself
-   May require copying the value wrapped

Those may be mitigated by leveraging unused bit patterns, passing a view of the
value, or inverting control for of `for` block statements.
