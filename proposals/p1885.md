...
need to cache the last value, to allow for `*it` to be called, and to be called
more than once.
...
class IterateIterator {
 public:
  static auto Invalid(const Iterable& container) -> IterateIterator<Iterable> {
    return IterateIterator<Iterable>(container, std::nullopt);
  }

  IterateIterator(const Iterable& container,
                  std::optional<typename Iterable::CursorType> cursor)
  ...
  auto operator*() const -> const typename Iterable::ElementType& {
    assert(cursor_);
    return value_;
  }
  auto operator++() -> IterateIterator<Iterable>& {
    next();
    return *this;
  }
  // `true` if both are equal or invalid/empty
  auto operator==(const IterateIterator<Iterable>& rhs) -> bool {
 ...
 private:
  void next() {
    assert(cursor_);
    if (const auto val = container_->Next(*cursor_); val) {
      value_ = *val;
    } else {
      cursor_ = std::nullopt;
    }
  ...
  const Iterable* container_;
  std::optional<typename Iterable::CursorType> cursor_;
  typename Iterable::ElementType value_;
};
...
class Iterable {
 public:
  ...
  virtual auto NewCursor() const -> CursorType = 0;
  virtual auto Next(CursorType& cursor) const -> std::optional<ElementType> = 0;
...
class MyIterable : public Iterable<int, std::string> {
  auto NewCursor() const -> CursorType override { return 0; }
  auto Next(CursorType& cursor) const -> std::optional<ElementType> override {
    if (cursor < 10) {
