...
-   [Details](#details)
    ...
    -   [Consumable](#consumable)
        ...
        -   [TODO](#todo)
    -   [Iterable](#iterable)
        -   [Range-based `for` behavior](#range-based-for-behavior-2)
        -   [Interface and usage](#interface-and-usage-2)
...
We propose three interfaces:

-   [**Indexable**](#indexable): Implements index-based random access
-   [**Iterable**](#iterable): Implements iterator-based sequential access
-   [**Consumable**](#consumable): Implements single-use sequential access

Users would benefit from providing an `external impl`, to limit class members
pollution.

## Details

This proposal covers support for 2 different type of containers:
...
In absence of strong requirements from the algorithm, this would require future
algorithms to:

-   A. explicitly support multiple interfaces: expressive, and specialize
    implementations based on type features (likely preferred)
-   or B. define adapters or implicit conversions to respectively simplify
    algorithms implementation or end-user usage

### Indexable

`Indexable` represents a container that supports index-based random access.

#### Range-based `for` behavior

-   Get container size (`size`)
-   Iterate from `[0;size[`, returning current item at each iteration
...
class MyArray {
    ...
    impl as Indexable where .ElementType = i32 {
        fn GetSize[me: Self]() -> u32 { ... };
        fn GetAt[me: Self](pos: u32) -> ElementType { ... };
    }
}
...
A related added value would be the support for index-based access and assignment
with the `[]` operator, as a 1D array, for `Indexable` types. In addition to
basic single-element operations, this opens the door for future more advance
uses, as proposed in
[PR-1928](https://github.com/carbon-language/carbon-lang/pull/1928).

The type-checker would however not be able to validate `Indexable`-based array,
in comparison to fixed arrays:

```rust
fn Foo[T:! Indexable](array: T) {
    array[1] = array[2]; // May or may not exist
}
```
...
ca be read (consumed) once, and are not expected to be accessible anymore after
usage.
