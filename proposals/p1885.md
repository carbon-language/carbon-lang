...
range-based iteration with `for`. The proposed solution exposes an interface
...
-   Iterating over r-value container
...
-   Get an `Iterator` type with `GetIterator()`
-   Call `Advance() -> bool`
    -   Break the loop if the result is `false`
-   Call `Get()` or `GetMutable` and return the element
...
interface Iterator {
    ...
    // Return optional element. Needed in the broader context of
    // Iterators and C++ interop
    fn Get[me: Self]() -> Optional(ElementType);
    // Advance our iterator
    fn Advance[me: *Self]() -> bool;
    // Advance and return optional element
...
```carbon
fn Get[addr me: *Self]() -> Optional(ElementType) {
