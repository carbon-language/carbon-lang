...
adaptation to interoperate with the C++ iterator model. The adapter would
be a Carbon type that satisfies the Cursor interface, implemented in terms
of a C++ iterator.
...
C++ for any Carbon type that implements the `Iterate` interface, in
addition to overloads of the `*`, `++`, and `!=` operators for the types returned.
