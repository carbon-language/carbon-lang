...
Carbon code is expected to include documentation comments. In addition to being
consumed directly by readers of the code, such comments are expected to be used
by documentation generation systems, and as part of the feedback given during
code completion in an IDE. Specifying such comments as part of the language
definition allows a consistent interpretation of the comments, and a consistent
attachment of the comments to entities, to be provided for Carbon programs.

**Open question:** Use of documentation comments by tools would be enhanced by
specifying a concrete format for documentation comments. This proposal doesn't
provide such a format, but a future proposal should.
...
(such as a multi-line string literal) is an error.
...
// This ends the block comment that began with `//{ temp`.
//}
```
...
// The next line is an error because it has trailing text (" temp"), and that
// text does not match the trailing text of the opening line (" mismatched").
//} temp
```
...
For the case of an "end namespace" comment, the underlying problem is solved in
a different way, by not providing a syntax for namespaces as delimited scopes.
