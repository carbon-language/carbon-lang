...
features. Qhether it should have only untyped byte buffers, only typed unions,
...
-   It may be somewhat less efficient (or impose more load on the optimizer),
    because it requires instantiating the enclosing `Choice` type (presumably
    including a discriminator field), rather than merely passing the appropriate
    alternative into the continuation.
