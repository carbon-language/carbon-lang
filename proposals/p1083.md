...
-   [Alternatives considered](#alternatives-considered)
    ...
    -   [Guarantee that all overflow errors are trapped](#guarantee-that-all-overflow-errors-are-trapped)
    ...
    -   [Use a different operator for string concatenation](#use-a-different-operator-for-string-concatenation)
...
-   `+` and `-` mean addition and subtraction, as in mathematics. Unary `-`
    forms a negative value -- an additive inverse. Sometimes, unary `+` is
...
However, due to the ubiquity of this notation among programming languages, any
...
`a + b % c * d` is an error, not `a + (b % c) * d`.
...
-   [Interoperability with and migration from existing C++ code](/docs/project/goals.md#interoperability-with-and-migration-from-existing-c-code)
    ...
    -   Making similar choices for the behavior of signed and unsigned types
        will reduce friction between C++ and Carbon.
...
should be unsurprising is programmers with C++ or Rust experience. Therefore we
choose the conventional symbol set.
...
scenarios where unbounded undefined behavior on integer arithmetic is not
...
-   Operations such as negation and division can increase the width of the
    ...
    -   Give integer types a range of values rather than simply a bit-width. For
        ...
        represent [-2<sup>31</sup>+1, 2<sup>31]. However, this would add
        significant complexity to the type system, and doesn't solve the full
        problem: `a / b`, where `a` is `i32`, has 2<sup>32</sup>+1 distinct
        possible values if `b` is unknown.
...
intentional wrapping indistinguishable. This would mean that static and dynamic
analysis tools can no longer assist the programmer in finding such bugs. Making
overflow issues programming errors shifts the problem left allowing it to be
caught earlier.
...
[guaranteeing a trap on overflow](#guarantee-that-all-overflow-errors-are-trapped).
...
-   Better expression of developer intent. It is generally preferable to make
    invalid cases unrepresentable, and if negative numbers are invalid, then
    ...
    by unsigned types is worthwhile, and supporting only the case of a range
    restriction to [0, 2<sup>N</sup>-1], but not any other range, seems
    arbitrary and unlikely to address the desire to capture intent and make
    invalid cases unrepresentable.
...
-   Harms the educational argument that `iN` should be used for integers, and
    `uN` should be used for special cases where modular arithmetic is desired.
...
There is no single canonical total order for that ring, and nor does it support
...
-   Unfamiliar to people coming from C++ and Rust.
...
both existing programming languages and common high-school mathematical
education treat multiplication and division as the same, left-associative,
precedence group means that it's unlikely to be a significant burden to expect
Carbon developers to remember this precedence rule.
...
### Use a different operator for string concatenation

We could attempt to use `+` only for operations that are adding numbers, and
compared to similar languages.se a different syntax for string concatenation.

Advantages:

-   Can perhaps give more guarantees about addition, such as that the literal
    `0` will implicitly conveert to any addable type and is the additive
    identity.
-   Avoids overloading between operations that might appear near to each other
    with very different meanings, or that might have different meanings in other
    languages: `"1" + "2"` is `3` rather than `"12"` in some scripting
    languages.

Disadvantages:

-   The `+` operator is familiar to C++ developers and any other operator choice
    would be a surprise.
-   There is no formal definition of what constitutes a number. Indeed, in
    abstract algebra, sequences of letters from some alphabet are often treated
    as number-like objects, and the operator in a monoid (such as string
    concatenation) is often written as '+'.
