...
-   Operations such as negation and division can increase the width of the
    ...
    -   Give integer types a range of values rather than simply a bit-width. For
        ...
        represent [-2<sup>31</sup>+1, 2<sup>31</sup>]. However, this would add
        significant complexity to the type system, and doesn't solve the full
        problem: for example, `a / b`, where `a` and `b` are `iN`s, has
        2<sup>`N`</sup>+1 distinct possible values, which still forces the
        result into a type with a higher bit-width. This is especially
        surprising because integer division is usually expected to make a number
        smaller!
...
We could be more mathematically pure by refusing to implement `<` and friends
for `uN`, and similarly refusing to support `/`.
