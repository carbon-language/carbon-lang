...
bit, only odd numbers have
[multiplicative inverses](https://en.wikipedia.org/wiki/Modular_multiplicative_inverse),
and so division by even numbers is not well-defined. We could be more
mathematically pure by refusing to implement `<` and friends for `uN`, and
similarly refusing to support `/`.
