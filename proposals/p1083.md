...
overflow issues programming errors allows problems to be caught earlier.
...
-   Better expression of developer intent. It is generally preferable to make
    ...
    by unsigned types is worthwhile. Moreover, while a restriction to
    non-negative values is common, supporting only the case of a range
    restriction to [0, 2<sup>N</sup>-1], but not any other range, does not do a
    good job of addressing the general desire to capture intent and to make
...
We could attempt to use `+` only for operations that are adding numbers, and use
a different syntax for string concatenation.

Advantages:

-   Can perhaps give more guarantees about addition, such as that the literal
    `0` will implicitly convert to any addable type and is the additive
    identity.
