...
syntax, `$&lt;char>`, where `&lt;char>` is one of `u`, `o`, or `f`, will be
added to designate which version of the function should be called. If this
calling syntax is not present, the checked version is called. The `$&lt;char>`
calling syntax is invalid for non-`$checked` functions.
...
`$&lt;char>` calling syntax is used. There is no implicit change to what is
called based on the version of `Get()`.
