...
deduced arguments, and then pattern matching is used to bind parameter patterns to
argument values.
...
-   Argument deduction is performed by comparing the declared parameter types
    against the actual argument types and deducing values for implicit
...
-   Then, for each binding in the explicit parameter list in turn, all argument
    ...
    -   If the parameter is a checked `:!` binding, the argument expression is
        converted to a symbolic value phase expression of the type of the
        binding.
...
Note that the types of the call arguments are modeled as an interface
parameter. This permits the call interface to model function overloading.
However, the return type is an associated type, not a parameter -- we do not
permit overloading on return types, and we don't want type information to
propagate from the context in which a call expression appears inwards to the
call.
...
### Overloading on expression category

We should consider whether calls can be overloaded on the expression category
of the callee. Rust provides separate `Fn`, `FnMut`, and `FnOnce` to
distinguish between different ways of passing the callable object into an
overloaded function call, and C++ permits `operator()` to take `*this` as
`const`, non-`const`, or even `&&`.

This would most likely take the form of multiple `Call` interfaces to handle
different kinds of `self` parameter, modeled in a similar way to the
`IndexWith` and `IndirectIndexWith` interfaces used for subscripting.
