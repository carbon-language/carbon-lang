...
`(T as Interface).Member` or always mean `(typeof(T) as Interface).Member`, but that
did not allow aliases to work as expected. We wanted an alias to an interface method or
class function defined inside of a class to be usable as a method or class function
member of the class.
...
We want `B.G` to act like a method of `B`, like `B.H`, and `B.T` to act like a static
class function of `B`, like `B.U`. We could have accomplished this by requiring those
aliases to be written in a different way, for example `alias G = (Self as A).F`, but
needing to do that was surprising. We made it work by making the interpretation of
`T.(Interface.Member)` depend on whether `Member` was an _instance_ member of
`Interface`, with an implicit `me` parameter, or not.
...
We considered making `type` an alias for an empty type constraint defined in
the prelude, `Core.Type`. While it is possible to define a constraint that is
