...
-   [Issue #508](https://github.com/carbon-language/carbon-lang/issues/508)
    ...
    self-typed.
-   [Issue #2360](https://github.com/carbon-language/carbon-lang/issues/2360)
...
-   A _facet type_ is a constrained or unconstrained version of `type`, such as
    an interface type or a named constraint type. We previously referred to a
    facet type as a type-of-type, but that is no longer accurate, as the type of
    any type is now by definition `type`.
...
-   For a simple member access `a.b` where `b` names a member of an interface
    ...
    -   If the interface member was found by searching a non-facet-type scope
        ...
        for `T as I`. For example, `MyClass.AliasForInterfaceMember` finds the
        member of the `impl MyClass as I`, not the interface member.
    -   Otherwise, `impl` lookup is not performed.
-   For a compound member access `a.(b)` where `b` names a member of an
    interface `I`, `impl` lookup is performed for `T as I`, where `T` is the
    type of `a`.
    -   For consistency, this rule is used regardless of whether `b` names an
        instance member.
...
One consequence of these changes is that there is no longer a simple way to find
an impl member given the name of the corresponding instance member:
...
fn PrintNamesOf(template A:! type, template B:! type, template C:! type) {
  alias Print = Named.PrintTypeName;
  // Previously called `(A as Named).PrintTypeName()`,
  // now calls `(type as Named).PrintTypeName()`.
  A.(Print)();
  B.(Print)();
  C.(Print)();
}
...
It's unclear to what extent this will be an ergonomic issue, but it may be
minimal given the limited places in which the name of an interface member can
appear. If it is significant, we can look into adding some syntax to support
this operation.
...
[open discussion](https://docs.google.com/document/d/1tEt4iM6vfcY0O0DG0uOEMIbaXcZXlNREc2ChNiEtn_w/edit#heading=h.lecxe8qywdxa).
