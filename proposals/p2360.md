...
-   [Details](#details)
    ...
    -   [Things not being changed](#things-not-being-changed)
-   [Rationale](#rationale)
...
look into the type of `x`, which symbolically evaluates to the value `T` of type
`Type`, and will look at that value `T` to determine where else to look. Because
`T` is symbolically the name of a generic type parameter, we will look in the
type of `T`, which is `Printable`.

### Things not being changed

This proposal does not introduce the name `Type`, which remains provisional. See
[issue #2113](https://github.com/carbon-language/carbon-lang/issues/2113) for
discussion of how things like `Type` should be named. At this time, that issue
is still open, but it might conclude that `Type` is renamed `Core.Type`, with a
keyword alias `type`.

Under the [current design](/docs/design/generics/details.md#named-constraints),
`Type` is the kind with no requirements, and an equivalent kind can be declared
with `constraint MyVersionOfType {}`. This proposal does not change this
decision. Because such a constraint `MyVersionOfType` is the same constraint as
`Type`, a value `T:! MyVersionOfType` is a type under this proposal.
