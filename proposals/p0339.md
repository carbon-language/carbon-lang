...
other languages). In general `var` appears to be a more consistent choice.
...
-   Carbon must have a syntax for variable declaration.
-   Using `<type> <identifier>` ordering closely mirrors C++.
    -   Although `<identifier>: <type>` is common, we are keeping a
        C++-consistent style to start with, and will switch if data supports it.
-   Adding `var` improves parsability of Carbon.
