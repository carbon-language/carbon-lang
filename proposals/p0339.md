...
```bison
statement:
  "var" type identifier optional_assignment;
| /* pre-existing statements elided */
;

/* Type expressions should be expected to broaden; an identifier is minimal. */
type:
  "auto" |
  identifier
;
...
[colon between type and identifier](#colon-between-type-and-identifier),
particularly as noted for [use in pattern matching](#use-in-pattern-matching),
that syntax should be changed to remove the `:`. This will not reflect approval
of that syntax, only that we want to keep example code consistent. For example,
replacing `fn Sum(Int: a, Int: b) -> Int;` with `fn Sum(Int a, Int b) -> Int;`
and `case (Int: p, (Float: x, Float: _)) if (p < 13) => {` with
...
other languages). In general `var` appears to be a more consistent keyword.
...
We are discussing having the spacing of `*` be important (this may or may not be
done). In the first statement, `*` is a unary operator and so `T*` is the type
and `x` is the identifier. However, in the second statement, `*` is a binary
operator and so `T * x = 3` is the type, and `y` is the identifier; the
resulting compiler errors may be confusing to users.
...
In the first and second statements, the types and identifiers are the same. In
the second statement, the reader should easily be able to look at the code and
see the type is unusual. In the third statement, `T *` is now the type, although
likely a compiler error due to the space before `*` leading it to being a binary
operator with no second argument — even then, the compiler may be able to easily
and specifically recommend removing the space.

In general, it should be expected that adding `:` reduces ambiguity, and allows
...
other approaches may work sufficiently or better. No rationale is given for the
use of `:` in the proposal, and it dates to a period when `:` was used
provisionally in Carbon syntax: it would be best to examine the choices more
before reaching conclusions. Per [syntax ambiguity](#syntax-ambiguity), it is
expected that `:` has some advantages, but may not turn out to make a compiler
difference due to prevailing constraints on type expression syntax.

As things stand, the provisional pattern matching syntax in the language
overview should be updated for consistency with this proposal. Details should be
resolved as part of the eventual pattern matching proposal, but if changes are
needed to add a separator, the `var` syntax should be updated to remain
consistent. The precise form of that implementation will be part of normal
Carbon evolution.
...
now is a study from an unpublished bachelor's project from Germany. The study
