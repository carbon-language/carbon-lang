...
used `:` as a separator. Note that no rationale for `:` is given in that
proposal, and it dates to a period where we were using `:` in Carbon syntax, so
it's unclear to what degree it was adopted for consistency. In part the use of
`:` allows for types to be parsed more easily, but it is generally true that `:`
reduces [syntax ambiguity](#syntax-ambiguity).

The pattern matching syntax in the language overview should be updated for
consistency with this proposal. Details should be resolved as part of the
eventual pattern matching proposal, but if changes are needed to add a
separator, the `var` syntax should be updated to remain consistent. The precise
form of that implementation will be part of normal Carbon evolution.
