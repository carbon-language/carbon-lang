...
constraint W {
  ...
  // This requires an impl lookup where the query interface is NeedsY
  // and the parameter is Self (which is of type W).
  // Under the new rule, W is identified because it has begun being defined.
  // The lookup for 'NeedsY(Self)' can now see that 'Self' implements 'Y'
  // due to the previous 'require' statement.
  require impls NeedsY(Self);
}
```

In this example, identifying `W` while it is being defined allows the lookup for
`NeedsY(Self)` to succeed because the compiler knows `Self` (of type `W`)
implements `Y` from the previous `require impls` statement.
