...
-   The primary motivation is to provide _access_ to both features.
    -   Leads consider it important to support (2) separate from (1), even
        ...
        -   For example, this means `extern class C;` might mean there's a
            declaration in a separate library in order to resolve a dependency
            cycle, or it might mean that `C` is only `extern` in order to
            require direct imports. If a developer considers it important to
            disambiguate how `extern` is being used, they must add comments.
        -   Using `extern` for (2) without (1) means any imported library could
            add an `extern library "<owning_library>"` declaration without
            further coordination with the owning library. Again, this is not
