...
-   The primary motivation is to provide _access_ to both features.
    -   Using `extern` for (2) without (1) still leaves it possible for misuse
        of (1): that is, a separate library could add a forward declaration
        unexpectedly. Leads don't expect this to be a problem.
...
-   Adding the owning library with `extern library "<owning_library>"` will
    hopefully produce better diagnostics, better human readability, and
