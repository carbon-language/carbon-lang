...
permit value binding for anything where `T` permits reference binding:
...
`i32`, which an `InRange` value can implicitly convert to.
...
class I32Range {
  ...
  extend api EmptyExtension(I32Range);
}

// Because of `extend api`, `I32Range.Three` is equivalent to
// `ThreeExtension.Three`.
Assert(I32Range.Three() == 3);

var r: I32Range = {.low = 2, .high = 5};
// Because of `extend api`, `r.Empty` is equivalent to
// `r.(EmptyExtension(I32Range).Empty)`
...
[This comment on proposal #3720](https://github.com/carbon-language/carbon-lang/pull/3720/files#r1527089564)
