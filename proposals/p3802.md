...
to indicate that lookups should be performed in another constraint,
and to add a requirement that `Self` implements the other constraint.

For some use cases, we want to be able to add to name lookup into a type
...
impl forall [__Self:! I & HasMix]
  Mix(__Self).plus as ValueBind(__Self) where .Result = i32 {
  fn Op[self: Self](x: __Self) -> Result {
    return x.GetMixinValue().plus;
  }
}
impl forall [__Self:! I & HasMix]
  Mix(__Self).plus as RefBind(__Self) where .Result = i32 {
