...
which field of a union is live (if any), and user code is responsible for
ensuring that a union field is only accessed or destroyed while it is live, and
only created if no union field is live. This is motivated by Carbon's
performance goals: embedding a discriminator in every union would be wasteful in
cases where the code can store the discriminator more efficiently elsewhere, or
track the live field by other means. Furthermore, a hidden discriminator field
would make it more difficult for developers to understand and control the layout
of their data in memory (much like the hidden vptr field in many C++ objects).
...
In this proposal, accessing a union member that isn't live is out of contract.
This is unlike C++, which permits assigning to an active member as a way of
making it active, and permits reading from inactive members when they're part of
a common initial sequence, as well as in a much broader but more nebulous set of
...
First, it's tempting to think of a union with no live member as equivalent to an
uninitialized field. If that were the case, initialization shouldn't leave the
union without a live member unless the initializer explicitly requests it using
a syntax like `.union_name = uninit`, which would require us to name the union.
...
everything there is to know about the state of a union with no live member. In
particular, we know that it satisfies the preconditions for the `create:`
operation. Thus, a union with no live member is much more like a disengaged
optional value than an uninitialized value.

To put the point more pragmatically, we require `= uninit` to be explicit for
types like integers because programmers have a strong default expectation that
an integer variable holds a meaningful value, and it is easy and commonplace to
write code that works correctly for any reasonable integer value, but will fail
badly if given an uninitialized integer value. The structure of Carbon unions
(in particular the separate destruction and creation steps) should discourage
any default expectation that unions always have a live member. Furthermore, it
will be all but impossible to write code that works correctly for any
"reasonable" union value, but fails for a union with no live member, because
there is absolutely nothing you can usefully do with a union that has an unknown
live member.
...
accesses to union members that aren't live, especially in hardened production
builds. Arguably, it might make sense to postpone adopting this proposal until
those issues are resolved, in case resolving those issues turns out to require a
