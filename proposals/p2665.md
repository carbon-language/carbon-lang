...
it harder to assess other than that Swift developers are generally happy with
the results. However, there are observable ways the behavior can lead to small
mistakes; these may may often be caught by the compiler, but will sometimes be
missed. For example:
