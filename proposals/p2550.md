...
-   `library "Bar"` (`api` | `impl`) `;` for a library that is part of the
    `Main` package.
...
-   `import Foo` [`library "Bar"`] `;` to import package `Foo` library `"Bar"`.
    This introduces the name `Foo` in the current source file as a name for that
...
libraries from the `Main` package from any other package.
...
divergence, and it seems preferable to leave only one syntactic choice here in
