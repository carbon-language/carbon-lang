...
Note that we may revisit some of these decisions in the future. For example, we
could say that `expr` being dependent doesn't mean `if expr then a else b` and
`expr as T` are dependent, but we are using a conservative rule for now.
