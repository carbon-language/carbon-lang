...
reside. Note that if the word does not have a predefined meaning, this will
result in an error, as the program will not match any known grammar production.
...
It is an important property that identifiers from the Carbon prelude and words
with predefined meanings are essentially equivalent in this scheme: in
particular, this means that adding identifiers to the prelude can be a
non-breaking change in the same way that adding words with predefined meanings
is a non-breaking change. Note that in order to ensure this property holds, we
also need to exempt identifiers in the prelude from any name shadowing
...
We may want to disallow using some words with predefined meanings as identifiers
in cases where there is no evolutionary benefit to permitting such use. For
example:
...
There may be words that we wish to support as keywords but never intend to
reserve. For example, we might choose to never reserve the word `yield`, on the
basis that it is a useful noun that is important in several industries and
domains such as finance and manufacturing, but still treat it as a keyword in
the context of generators.

When a word is given new predefined meaning, we should decide and publicize a
choice of whether we intend to eventually reserve that word, in order to give
the Carbon community time to transition. Implementations may wish to add a
warning at that time for uses of the word as an identifier, and upgrade that
warning to an error once the word becomes reserved.
...
a word token as a keyword or identifier can depend on the contents of other
files.
...
parsing. Tools that perform rewrites should be careful to not introduce words
with predefined meaning as identifiers in general, but if such identifiers are
introduced in new code, the Carbon formatter should rename them to raw
