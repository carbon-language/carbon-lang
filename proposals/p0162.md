...
The keyword for methods in this proposal is `method`, which is one of the
choices considered in the structs proposal
[#98](https://github.com/carbon-language/carbon-lang/pull/98). That proposal
discusses the alternative of using `fn` for both function members and method
members, and differentiating between the two based on whether the type of the
first parameter is `Self`. The argument for using `method` is that it more
directly captures intent.
