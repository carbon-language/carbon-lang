...
pattern matching, and a sample of operators on Booleans and integers. The main
syntactic categories are `declaration`, `statement`, and `expression`.
...
occurrence, or whether the identifier is a field name. The period also enables
the addition of new keywords to Carbon without colliding with field names.
...
grammar for function definitions does not currently include implicit parameters,
