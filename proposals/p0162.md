...
syntax is straightforward. However, the parsing of a function call
deserves a little explanation. Recall the grammar rule:

    expression: expression '(' field_list ')'
    
The result of parsing the expression is placed in the `fun` field of
`call`. If `field_list` only contains a single unlabeled entry, then
the result of parsing that entry is placed in `arg`.  Otherwise a
`tuple` expression is created from the result of parsing `field_list`
and the tuple is placed in the `arg` field.

The `field_list` also deserves some explanation. 
The fields can be labeled with the grammar rule:
