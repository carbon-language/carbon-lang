...
Discussion has indicated potential utility in syntax to make the expectation of a trivial destructor _explicit_. This would allow a declarative way of ensuring no member accidentally caused a type to have non-trivial destruction.
...
`fn destroy` was preferred because it builds on existing `fn` syntax, The
ambiguity of the out-of-line form (`fn destructor MyClass [...]` is a
significant
