...
-   C++'s `while` loop style is common and desirable to stay consistent with.
-   Initializing variables in the `while` is not immediately necessary to have
    `while` be useful; we may even choose to omit it. It's not necessary to
    decide immediately.
    -   Some discussion about this is in
        [issue #425](https://github.com/carbon-language/carbon-lang/issues/425).
