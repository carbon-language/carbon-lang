...
-   Expensive both in effort expended and in wall clock latency to make
    ...
    -   These costs are not proportional. The high constant factor of cost
...
-   The assigned lead should ensure that at least three contributors are
    generally supportive and react with thumbs-up, or explain why the proposal
    needs to be deferred and close the PR.
...
-   Optionally, the assigned lead can file a blocking issue for a one week final
    ...
    proposal.
-   The proposal PR can be submitted once it has an LGTM from the assigned lead
    and all blocking issues have been decided, and those decisions incorporated.
...
No specific composition of the directory is proposed here. Instead, it is
...
_did_ end up happy with the resolution. At the end of the day, while its fine to
