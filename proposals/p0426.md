...
-   Expensive both in effort expended and in wall clock latency to make
    ...
    -   These costs are not proportional. The high constant overhead
        places a relatively higher cost on simple, uncontroversial proposals.
...
-   The assigned lead should ensure that at least three contributors (possibly including the lead) are
    generally supportive and react with thumbs-up.
-  If the lead chooses to defer or reject the proposal, they should explain why and close the PR.
-   Once the thumbs-up are present and the assigned lead finishes code review,
    the lead should LGTM the PR. Any outstanding high-level concerns should be
    handled with blocking issues.
-   Optionally, the assigned lead can file a blocking issue for a one week final
    comment period when they LGTM if it is both useful and important for the
    proposal.
-   The proposal PR can be approved and submitted once it has an LGTM from the assigned lead,
    all blocking issues have been decided, and any related decisions are incorporated.
...
-   Incorporate any changes needed based on the resolution of blocking issues.
    ...
    with that direction.
-   When you both have an LGTM from the assigned lead and the last blocking
    issue is addressed, submit!
...
No specific composition of the experts directory is proposed here. Instead, it is
suggested to collaboratively build that with interested parties adding
...
A proposal PR should have a descriptive title and easily understood summary
initial comment. Authors and leads are encouraged to edit both as necessary to
ensure they give the best high-level understanding of the proposal possible. The
proposals should then use the template markdown file to describe itself fully.
This template will have an additional section to contain the rationale for the
...
PRs that are in "draft" status in GitHub are considered works-in-progress. Check
with the author before spending time reviewing these, and generally avoid
distracting the author with comments unless they ask for them. The proposal may
be actively undergoing edits.
...
_did_ end up happy with the resolution. At the end of the day, while it's fine to
resolve an issue that _everyone_ actually ended up agreeing about (maybe once
...
-   Would provide defense against the failure modes seen in other language
    evolution processes.
