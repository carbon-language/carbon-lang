...
and `const` methods in C++. This is tricky because those constructs in C++ _do_
allow the address to be taken. As a consequence, Carbon will synthesize an
object and address for value expressions when necessary for interop. This
mechanism may also be used as an escape hatch for value expressions, but that is
[currently not proposed](#value-expression-escape-hatches).

It may seem risky to expose Carbon's value expressions to `const &` parameters
in this way, as C++ allows casting away `const`. However, in the absence of
`mutable` members, casting away `const` does not make it safe to _mutate_
through a `const &` parameter (or a `const`-qualified method). C++ objects can
be _declared_ `const`, and those objects are allowed to be implemented in
readonly memory and assumed to not mutate even if escaped. And `const &`
parameters can bind to these `const`-declared objects. As a consequence,
actually mutating through a `const &` parameter (perhaps after casting away
const) is a fundamentally unsafe operation in C++, and one that
[breaks in real implementations](https://cpp.compiler-explorer.com/z/KMhTondaK).
As a consequence, there is strong pressure that C++ code operate with `const &`
parameters in a way that will be fully compatible with Carbon's value
expressions.
...
-   C pointers can be null, the ["billion dollar mistake"](null-mistake),
    Carbon's cannot be. Carbon requires null-ness to be explicitly modeled in the
    type system and dealt with prior te dereference.
-   C pointers can be indexed even when not pointing to an array. Carbon's
    pointers are to singular objects. Carbon handle indexing with a separate
...
building in pointers to its foundations, it is building in the least problematic
...
-   `let` and `let mut`, based on the Rust names
    -   While nicely non-inventive, there was both concern that `mut` didn't as
        ...
        not being as obviously or unambiguously a good default with `let`.
    -   The `mut` keyword feels more fitting in its use with mutable borrows
        than as a declaration introducer the way it would work in Carbon. For
        example, it doesn't convey a desired aspect here of allocating storage.
-   `val` and `var`
...
void SomeFunction(const int &id) { // Here `id` is an immutable view of some
...
function accepting an immutable view: SomeFunction(other_id);

// We can also pass ephemeral values: SomeFunction(other_id + 2);

// Or values that may be backed by read-only memory: static const int fixed_id =
42; SomeFunction(fixed_id); }
...
class MyThreadCompatibleType { public: // ...
...
multiple threads with a shared `thing`. int thing_size = thing->Size();
