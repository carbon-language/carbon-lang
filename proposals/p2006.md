...
-   [Proposal overview](#proposal-overview)
    ...
    -   [Pinning R-values and `const &` interop](#pinning-r-values-and-const--interop)
...
R-values are formed by literal expressions (`42`), expressions that evaluate to
a R-value `1 + 2`, names introduced by readonly patterns, and the conversion of
a L-value into a R-value.

Pointers are used to indirectly refer to L-values. L-values are only formed
through variable patterns and the dereferencing of a pointer -- there are no
references in Carbon.
...
a R-value cannot be used. A `const` L-value converts to a non-`const` R-value --
`const` is only meaningful when applied to an L-value's type.

### Pinning R-values and `const &` interop

There are two use cases that require extending what you can do with a R-value.
The first is a specialized, rare, but important use case of small-size-optimized
containers and similar types with an API that _must_ expose an interior pointer
despite being an R-value. The second is passing R-values to C++ `const &`
parameters for interoperability.

Both of these use cases require exposing or materializing storage and producing
an address for a R-value. This is called _pinning_ in Carbon. A R-value in
Carbon that is pinned produces a pointer to a `const` L-value whose lifetime is
that of the pinned R-value: the scope of the name for a named R-value or the
lifetime of a temporary produced by an expression for a computed R-value.
Repeatedly pinning the same named value also consistently produces the same
L-value with a stable address.

TODO: Mention (briefly) lifetimes.
...
that Carbon dereferences to turn into a L-value.

Indexing in Carbon will also dereference and form L-values. The syntax is
...
in a raw pointer that the language dereferences to form a L-value. Because this
syntax always forms L-values, it is not suitable to overload for things like
forming a slice or span from an underlying container.
...
> FIXME: Reconsider and see if we can rewrite based around customized R-value
> representations.
