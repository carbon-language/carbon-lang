...
We have several reasons we want character literals in Carbon:

-   **Common in C++:** Looking at Sourcegraph's search, we see
    ...
    results for `'` in C++ (some of these are in comments -- a search
    limitation). By contrast, looking at something like the `%` operator, we see
    [5.3 million](https://sourcegraph.com/search?q=context:global+lang:c%2B%2B+count:10000000+/+%25+/&patternType=keyword&sm=0).
    While not as frequent as some C++ syntax, it is still fairly common.
...
-   **Block string literal use:** We also already have another use for single
    quotes in Carbon:
    [block string literals](/docs/design/lexical_conventions/string_literals.md).
    Repurposing single quotes away from character literals would require also
    finding a new syntax for block string literals.
