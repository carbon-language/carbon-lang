...
its `auto` variable rules, which are defined in terms of template argument deduction and
...
As a consequence, direct recursion is rejected: that is, a function with an `auto` return type may not
call itself.
