...
-   Multiple C++ types map to one Carbon type only in the cases when these are
    ...
    for overloading.
-   `floatN_t` types won't be supported in Carbon at this point and may be
...
-   All of the mappings are direct mappings without any performance overhead.
...
Carbon may not have a C++ mapping for `i256`/`u256`.
...
-   Map `f32` <-> `float32_t` and `f64` <-> `float64_t`
    -   Advantages: follow the same analogy as for the integer types (`iN` <->
        `intN_t`)
    -   Disadvantages:
        -   `floatN_t` types are new types since C++23 and this won't be
            possible for older C++ versions.
        -   `floatN_t` types are not aliases for the standard floating-point
            types (`float`, `double`, `long double`), but for extended
            floating-point types, so type conversions will be needed for the
            standard types.
...
-   `std::byte`
-   `std::nullptr_t`
-   `void*` - will this be supported in Carbon?
