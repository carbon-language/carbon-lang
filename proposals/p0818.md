...
alternative is it gives a convenient way of polluting the members of `T` with
the members of `B`, however it wasn't clear how common that use case would be.
Furthermore, it introduced an inconsistency with other associated types. For
example:

-   `T:! Container where .ElementType == i32` means that `T.ElementType` has
    type `i32`.
