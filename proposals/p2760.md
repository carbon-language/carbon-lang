...
-   Adapters are now a special case of classes, not a distinct top-level
    declaration.
-   Classes with `adapt` still may not contain anything that was previously
    forbidden for adapters: no data members, no virtual methods, no
...
-   Syntax for an `adapt` declaration inside a class body is:
...
interface I {
  ...
  // Again the `where` clause can be separated
  // into a separate `require` declaration.
...
-   Syntax for a `require` declaration in an interface or named constraint:
    ...
    > `require` _where-constraint-expression_ `;`
...
FIXME: Question-for-leads issue does state that a class will treat a mixin
syntactically like a data member for layout purposes.

-   With this proposal, base class declarations appear in the body of the class
    definition, like data members, so decision of whether mixins are more like
    base classes or data members of issue
    [#1000: Mixins: base classes or data members?](https://github.com/carbon-language/carbon-lang/issues/1000)
    is less significant. Like base classes, the mixin syntax begins with
    `extend`. Like data members, a mixin declaration can appear multiple times
    and intermixed with data member declarations.
-   Layout of the memory of an object reflects the order of the declarations in
    the class body.
