...
In addition FIXME Mixins use cases and syntactically

FIXME: declaration part of the definition is everything up to the opening `{` of
the definition body.
...
<!-- prettier-ignore -->
> [`extend`] `impl` [`forall` `[` _deduced-parameters_ `]`] _type-expression_
> `as` _constraint-expression_ (`;`|`{` _impl-body_ `}`)

FIXME: do we change the "internal" versus "external" terminology for
implementations?

### Class conditional implementation

FIXME: direct support for internal conditional implementation is removed.
