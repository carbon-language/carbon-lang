...
-   Escape sequences are introduced with a `\` character; the most common C and
    ...
    (`\177`), `\a`, `\b`, `\f` and `\v` are removed. `\U00NNNNNN` is replaced by
    `\u{NNNNNN}`. An escape sequence `\<newline>` is permitted in multi-line
    string literals, and results in no string contents.
