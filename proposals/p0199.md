...
-   Escape sequences, which permit string literals to include characters that
    are difficult to type, are ambiguous for the reader, or that would be problematic in some way (such as
...
(U+000A) added after lines that did not end with a `\<newline>`
...
compiler, but some file type indicators are understood by the language tooling (for example, syntax highlighter, code formatter) as indicating the structure of the string literal's content.
...
and [Rust](https://doc.rust-lang.org/reference/tokens.html), except that C-like
escaped newlines are supported to allow wrapping long strings to 80 columns.
