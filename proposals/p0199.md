...
| `\<line feed>` | No string literal content produced (block literals only) |
...
block string literals, new line are disallowed. All other characters (including
non-printable characters) are preserved verbatim. Because all Carbon source
files are required to be valid sequences of Unicode characters, code unit
