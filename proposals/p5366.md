...
for purposes of definition and `match_first` blocks.
...
Under the current rules introduced in proposal #1084 and modified in #3763, an
`impl` redeclaration must match syntactically.
...
forward declarations of entities comes from the
...
// Defining impl that was forward declared:
impl C1.C2.C3.(as I) { ... }
...
Notice that we don't know which form will be used until we see the open paren
(`(`) after the `.`.
