...
also look into `Z`. Immediately after the `extend` operation, name lookups from
inside the scope of `Z`, or lookups into `Z` from outside, should also look into
`Y`. In order to be able to perform lookups into `Z`, we require that `extend`
operations only target types that are complete.

 At the end of the definition of `Y`, we expect to be able to
