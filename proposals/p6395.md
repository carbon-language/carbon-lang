...
An `extend` declaration declares that the enclosing scope is extended by the
scope associated with any target scope named in the declaration. The `extend`
declaration requires that the target scopes named in an `extend` declaration
are complete, or if the target is a generic parameter, requires the type of the
parameter to be complete.

The scope of a facet type formed by a `where` declaration is extended by the
scope of its first operand. And the scope of a facet type formed by a `&`
operator is extended by the scopes of both of its operands. In either case, the
scope of the facet type is complete if every scope it is extended by is
complete. The facet type `type` is associated with an empty scope and is
complete.
...
This requirement functions recursively: Given an interface `A` that is extended
by another interface `B`. By naming `A` in an extend declaration, we require `A`
is complete. This provides that its entire definition is known, and thus its
...
must be complete where the `extend` declaration is written. As the generic
parameter is replaced by a more specific value, it must remain complete. This
includes forming a specific for the named scope if it is generic itself in order
to surface any monomorphization errors in the resulting specific.
...
completeness, as it requries `A(-1)` to be complete, which requires
...
This is based on the principle of [Code that is easy to read, understand, and
write](/docs/project/goals.md#code-that-is-easy-to-read-understand-and-write).
