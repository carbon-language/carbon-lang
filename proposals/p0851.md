...
At present it doesn't appear to mix bindings and expressions.
...
Whereas Swift doesn't support combining patterns, it's worth noting the use of
`auto` is used to indicate a value-binding pattern.
...
Carbon should offer variable type inference using:
...
Using the type on the right side currently results in a constant `IntLiteral`
value, whereas most languages would suggest a variable integer value. This is
something that will be considered as part of type inference in general, because
it also affects generics, templates, lambdas, and return types.
