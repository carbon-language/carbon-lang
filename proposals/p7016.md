...
The decision to use `static` for member variables is specifically because it
implies storage lifetime, which fits data member variables well. This
implication comes both from the origins of the term (static allocation of
storage versus dynamic allocation), and aligns precisely with how existing
languages use `static` on members variables, especially C++. This meaning also
aligns with common definitions of
...
However, static storage duration is the strong default for package- and
namespace-scope variables. The storage of these variables should be considered
"static" in the same way as static member variables, but the omission of the
keyword is appropriate because static storage is the strong default model in
those contexts. In contrast, the strong default for class members is instance
storage, so a keyword is needed to opt out of that default.

It is currently limited to classes, but if we allow such variables in other
contexts that don't have a strong default of static storage, like interfaces and
`impl` declarations, they should use the same `static` syntax.
