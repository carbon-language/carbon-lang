...
Treating methods as functions with an explicit first parameter means they can be
...
storage being a strong default for package- and namespace-scope variables. The
...
in the `()` list. Specifying the type of `self` with `: Self` is optional and
defaults to `: Self`.
...
`self` parameter. The method syntax of `obj.Method`, when `Method` names a
function with a `self` parameter, is just syntactic sugar for partially applying
the `obj` to the `self` parameter and calling the resulting function. This
includes the expression `obj.Method` itself being valid and being a callable
expression.
...
avoids stretching or diluting the meaning of `static` in Carbon. It let's us
...
### Treat `self` differently from _all_ parameters
