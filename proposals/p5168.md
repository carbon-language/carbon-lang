...
-   We accept the proposed resolution of
    ...
    -   Associated constants are assigned using `where .X = value;`
        declarations, with semantics matching rewrite constraints in `where`
        clauses.
...
-   Associated constant assignments should be encouraged to be placed in the
    ...
    -   The syntax for specifying the value of an associated constant in an
        `impl` definition is `where .X = value;`, just like a rewrite clause in
        a `where` expression, except that `where` in this case is an introducer
