...
Carbon aims to provide interoperability comparable to Java/Kotlin or
JavaScript/TypeScript interoperability, although it may be mildly less seamless.
It should be substantially smoother than C++/Java or C++/Rust interoperability.
...
data structure. Even where the expected interfaces differ, a trivial wrapper to adapt between the Carbon and C++ APIs will typically be sufficient. For many data structures, code will reasonably support multiple
...
exposed as yet another implementation in C++, and wrapped to match C++ idioms and even templates.
...
the syntax constructs may look different from C++ inheritance. This is considered
necessary to address cases where a C++ library API expects users to inherit from
a given C++ type.
...
C++ headers with declarations equivalent to Carbon declarations, that's a simple task compared to translating
arbitrary Carbon template code to C++. Supporting Carbon to C++ code
translations would be a high cost to achieve full parity for mixed toolchains,
and requiring bridge code for mixed toolchains is the likely result.
