...
-   C/C++ libraries compiled by a non-Carbon toolchain will be usable
    from Carbon, so long as they are ABI-compatible with Carbon's C/C++ toolchain.

-   The Carbon toolchain will support, as an option, generating a C/C++
    header and object file from a Carbon library, with an ABI that's suitable for
    use with non-Carbon toolchains.
...
and types to C/C++. This is necessary to allow Carbon to evolve things like
memory layout, whereas C/C++ interoperability may require an ABI-compatible
choice.
...
although the implementation may look different from C++ inheritance. This is
...
overhead, and/or lose information. Furthermore, if Carbon code calls a C++
function without suitable annotations or bridging, and that function exits with an
...
may choose a rule that conflicts with how C++ extends lifetimes of temporaries.
