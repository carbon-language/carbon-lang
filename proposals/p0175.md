...
-   [Proposal](#proposal)
    ...
    -   [Goals](#goals)
        ...
        -   [Make it possible to use the C/C++ ABI](#make-it-possible-to-use-the-cc-abi)
    -   [Non-goals](#non-goals)
...
we expect slow but meaningful evolution in implementations strategies.
...
expose C++ data structures as another implementation, and then focus on
providing wrappers if we want to make usage more idiomatic for Carbon.
...
We expect interoperability with C/C++ to be frequently used in Carbon, whether
it's C/C++ developers trying out Carbon, incrementally migrating a large C/C++
...
support interoperability with C/C++. To do otherwise would confine Carbon's
evolution to only allow features that were feasible to implement in C/C++, and
...
time, we should work to minimize the extra costs that interoperability exerts on
Carbon.

#### Practical safety guarantees and testing mechanisms

We should consider safety around interoperability code, and try to provide
mitigations where possible. However, we should expect that native Carbon code
will be easier to make safe than Carbon code that invokes C/C++. C/C++ code will
not benefit from the same set of safety mechanisms that we build for Carbon.
...
It's important to keep in mind that interoperability is, ultimately, a language
goal for Carbon. We should be thoughtful of what that means for the design, and
be willing to make trade-offs that harm other goals, so long as they offer
greater benefits for interoperability and Carbon as a whole.

### Goals

#### Support mixing Carbon and C/C++ toolchains

The Carbon toolchain will support compiling C/C++ code. We plan to take
advantage of this to customize the C/C++ compiler and enable cross-compatibility
of some more advanced features, such as calling Carbon templates from C/C++.
...
-   C/C++ ABI-compatible libraries compiled by a non-Carbon toolchain will be
    usable from Carbon.
-   The Carbon toolchain will support generating a C/C++ ABI-compatible library
    for use with non-Carbon toolchains.

Mixing toolchains restricts functionality to what's possible with the C/C++ ABI.
We will not constrain the Carbon toolchain's feature set to that, and so mixing
toolchains will result in degraded interoperability. This degraded
interoperability should still be sufficient for most developers, albeit with the
potential of more bridge code.

#### Minimize bridge code

The majority of simple C/C++ interfaces should be usable from Carbon without any
custom bridge code and without any runtime overhead. Carbon code should be able
to call most C/C++ code without any changes to the C/C++ code and without bridge
code. This includes using C/C++ code compiled with a non-Carbon toolchain, so
long as the C/C++ headers can be parsed by the Carbon toolchain.

In the other direction, we will minimize the amount of bridge code for C/C++ to
call into Carbon. However, some syntax should be expected to expose Carbon code
under a C/C++ ABI.

Our priority is that Carbon developers should be able to easily reuse the mature
ecosystem of C/C++ libraries provided by third-parties. We don't want a
third-party library's language choice to be a barrier to Carbon adoption.
...
Although we call these features "advanced", their use is endemic throughout C++
code, including STL. Support for such features is key to supporting migration
from C++ features.
...
Non-zero overhead conversions cannot be _required_, only _supported_. For
example, using `std::vector<T>` directly should be supported, and it should be
optional to undergo the overhead of copying it to the equivalent Carbon
structure.

#### Make it possible to use the C/C++ ABI

We will make it possible to use compiled code which conforms to the C/C++ ABI.
This will be true for both having Carbon code call C/C++ libraries, and having
Carbon code compile to the C/C++ ABI.

This does not mean that we will _exclusively_ use the C/C++ ABI, just that it
needs to be an option.

**Why**: Some C/C++ libraries are distributed as pre-compiled libraries.
