...
Carbon types will support inheritance from C++ types for interoperability,
...
-   Making conversions zero-overhead would require the Carbon type to mirror the
    ...
    pose an inappropriate constraint on Carbon's evolution to constrain Carbon's
    types more broadly.
...
Carbon doesn't need to provide interoperability support for exceptions. Throwing
exceptions across language boundaries will be unsupported. For example, Carbon
may immediately terminate the program if a C++ exception reaches a Carbon stack
frame.

#### Cross-language metaprogramming

Carbon's metaprogramming design will be more restrictive than C++'s preprocessor
macros. Although interoperability may be able to handle simple cases, such as
