...
-   C++'s range-based `for` loops are useful, and offer a good starting point
    for syntax. Use of `var` here is consistent with
    [the `var` proposal](https://github.com/carbon-language/carbon-lang/pull/339).
-   C++'s semisemi `for` loops are useful, but we want to investigate other
    options for the syntax.
    -   We believe that omitting semisemi will help avoid anchoring towards
        "keeping" it in Carbon.
    -   Omitting semisemi doesn't create a significant barrier to writing early
        example code.
    -   We may still add semisemi later.
