...
For example, here is a basic semsemi:
...
For loop syntax looks like: `for (` _variable_ `:` _expression_ `) {`
_statements_ `}`
...
-   Range literals. These are not proposed, but for an example seem in other
    languages, `0..2` may indicate the set of integers [0, 2).

### Executable semantics form

We presently don't have an expression that allows easily combining ways to
declare variables. This may become complex, particularly as we would likely
eventually allow use of multiple identifiers through tuples. Syntax should be
added to support this, proposed here as `variable_expression`.
...
statement:
  FOR '(' variable_expression : expression ')' statement
...
-   Swift does `for (i, item) : items.enumerated()`, having removed a
    `enumerate()` global function.
-   Rust does `for (i, item) : items.enumerate()`.
