...
-   [Subsetting the object for its unformed state](#subsetting-the-object-for-its-unformed-state)
    -   [Subsetting by fields](#subsetting-by-fields)
...
after moves. For example, we can consider an example set of control flows that
motivates this:
...
1. Types where there is an invalid state, and it can _optionally_ be queried,
   but the destructor will be a no-op without checking for that state.
2. Types where it is _necessary_ to query for an invalid state and skip
   meaningful logic in the destructor when in that state in order to use that
   state when unformed.
3. Types that have a valid state with a no-op destructor that they use when
...
We start with category (1), as our examples there are two important primitive
types: a non-owning pointer `T*` and `bool`. For the non-owning pointer `T*`
Carbon expects the null value to be invalid. And for bool objects, we expect to
have an entire byte of state, and so have a great deal of flexibility as only
two states will be valid.
...
`Optional(OwningPtr(T))` with an interesting destructor in some states.
...
2. Cheap, local run-time checking for use of unformed variables whenever
   possible beyond the compile time checks.
...
function must return a value of this struct type. When initializing a type `T`
with an unformed state to its unformed state, the fields of `T` that are in the
...
superset of the fields in UnformedInit.StructT. Note that these hardening
interfaces should not be relied on and shouldn't replace broad automatic
initialization, but can allow types to specifically add extra hardening where it
is most useful, and surface if there is an especially beneficial value to use.
...
The IsUnformed interface is implemented fully based on `UnformedInvalid`. It
also surfaces additional requirement on `UnformedHarden` when a type implements
both it and `UnformedInvalid`: the hardened state must also be an invalid state.
...
hardening _without_ supporting an unformed state. If that proves motivating, we
would either need a separate interface that is only used for hardening and
doesn't confer any unformed state semantics, or we would need to split the
hardening aspect of UnformedHarden apart into such an interface. Currently, the
suggestion is to have UnformedHarden be a superset because we expect this to be
the common case for types where an explicit hardening state is desired, and
separating it from UnformedInit would largely require types to implement two
things. In other words, we expect the blanket impl of UnformedInit in terms of
UnformedHarden to be a relatively common case.
...
way by modifying the adapt declaration with the unsafe keyword. Such an adaptor
has all the same properties as normal adapters, but the conversions between the
types are only available with unsafe as. These adapters can then explicitly
implement other conversion interfaces, but potentially do so _conditionally_ or
impose other restrictions such as only allowing them in one direction.
...
**Open question:** Alternatively, we could remove conversions from adapters
entirely. The adapter could use a different operation to actually perform the
conversion, and require users to explicitly write any of the main conversion
implementations desired.
...
**Future work:** Fully define how raw storage is represented for objects and the
...
fields in the UnformedInit.StructT type are available. All of the functions and
methods on T are available, but functions that safely operate on unformed
...
object being fully initialized. For example, this lets us describe the Self type
used in the `IsUnformed` interface for querying:
...
Types with an unformed state will either need to implement `IsUnformed`,
typically by implementing `UnformedInvalid`, or provide implementations of
`Core.Assign` and the destructor, if present, using `Core.MaybeUnformed(T)`.
When assignment or destruction directly use `Core.MaybeUnformed(T)` they
directly handle the unformed state, and any of their implementation details of
assignment and destruction might also need to use this type. Otherwise, when
those operations simply accept `Self`, the compiler will require an `IsUnformed`
implementation, use it to test for unformed state, and adjust the operation to
initialize (rather than assign) or skip destruction.
...
The first is useful when the `Core.MaybeUnformed(T)` object is in fact fully
initialized to continue handling that case. It can also be used when the
unformed state is arranged to be a valid initialization of the object and the
operation is merely reifying it as fully initialized.

The second is useful when the `Core.MaybeUnformed(T)` object is in fact in an
unformed state and a new object will be initialized into its existing raw
storage (because the destructor need not be run for such an object). The
language mechanisms needed for raw storage and initializing it this will be part
of specifying raw storage in a future proposal.
...
Because object above is known to be in an unformed state, we restrict how it may
be used subsequently: it may _only_ be used by a function that accepts the
object using the special type `Core.MaybeUnformed(T)` until used as a `ref`
binding to a non-destructor function with that type. The first use of the
address of the object by a non-destructor function is required to initialize the
object fully prior to returning, and the object is assumed to be a normal,
initialized object from then on.
...
is that practice these would likely be directly provided by builtin operations.
...
//
// Note that while there is a default `impl` of `UnformedInitConstant`,
...
class OwningPtr(T:! type) {
  ...
  impl as Core.IsUnformed {
    fn [self: Core.MaybeUnformed(Self)]() -> bool {
      // Note that `self.ptr` has type `Core.MaybeUnformed(PtrT))`, but
  ...
  // destructor calls to do that test themselves.
  fn destroy[ref self: Self]() {
    SomeDeallocationFunction(self.ptr);
  }
}
...
class Int(N:! IntLiteral) {
  ...
  impl as AssignWith(Int(N)) {
    fn Op[ref self: Core.Unformed(Self)](rhs: Int(N)) {
...
class OptionalOwningPtr(T:! type) {
  ...
  fn MakeEmpty() -> Self {
    return {.ptr = OwningPtr(T).(UnformedInvalid.Value)};
  }
  ...
  fn destroy[ref self: Core.MaybeUnformed(Self)]() {
    // We can directly access ptr as it is part of the unformed API subset.
    if (self.ptr.(Core.IsUnformed.Op)()) {
      return;
    }

    // We know this is a fully initialized object, so this is safe.
    let ref real_self: Self = self unsafe as Self;
    Assert(!real_self.ptr.(Core.IsUnformed.Op)());
    let ref real_ptr: OwningPtr(T) = real_self.ptr unsafe as OwningPtr(T);
    real_ptr.destroy();
  }
}
```
...
pointer won't have the Carbon unformed type annotations to identify them. We
...
We also specifically expect pointers and bool to implement IsUnformed, allowing
types that build on top of these to use them to model their own unformed state.
...
## Subsetting the object for its unformed state

The idea of explicitly selecting a subset of the object that participates in its
unformed state is important to dig into. The goal here is to enable types to
define potentially large portions of their state as remaining uninitialized for
efficiency without exposing code manipulating these types to initialization
unsafety. Making the subset explicit is important both to allow checking that
code is correctly accessing only the safe subset when operating on an unformed
object, and to enable sanitizers style diagnostics for the rest of the object.

### Subsetting by fields

We propose subsetting by fields rather than by a more complex approach such as a
[bitmask](link-to-alternative) because this is expected to be easy to implement
at low overhead, and meshes nicely with the struct literal syntax already
present in Carbon. This seems like an effective starting position, but we should
revisit it when introducing bitfields more fully to the language and determining
