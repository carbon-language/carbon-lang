...
2. Types where there is an invalid state that can be used as the unformed state,
   but the destructor must _check_ for that invalid state and skip meaningful
   logic to remain correct for objects in that state.
...
design type despite the ergonomic tradeoffs in the absence of an unformed state.
...
of unsafe code. Which ever semantic model a type uses for its unformed state,
...
these interfaces whet the members have implementations. Spelling that out and
...
`IsUnformed` implementation _must_ implement assignment and destruction with
...
language mechanisms needed for initializing raw storage will be part of
specifying raw storage in a future proposal.
...
may be used subsequently: it may be used freely with the special type
`Core.MaybeUnformed(T)`, but may only be used as the type `T` after it has been
used as a reference parameter of type `Core.MaybeUnformed(T)` to a call other
than the type's own destructor. That first call is required to initialize the
object fully.
