...
of unsafe code. This hardening only occurs to unformed or uninitialized objects,
never to well formed ones, and there is no restriction on whether both
...
be used subsequently: it be used freely with the special type
`Core.MaybeUnformed(T)`, but may only be used as the type `T` after it has been
used as a reference expression of type `Core.MaybeUnformed(T)` to something
other than the type's own destructor. That first reference expression to the
object is required to initialize it fully.
