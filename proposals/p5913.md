...
Our examples for category (1) are two important primitive
...
`Optional(OwningPtr(T))` (ignoring niche optimizations) with an interesting destructor in some states.
...
interface UnformedHardenInvalid {
  let StructT:! type;
  let Value:! StructT;
}
...
language mechanisms needed for initializing raw storage will be part
...
be used subsequently: it may be used freely with the special type
`Core.MaybeUnformed(T)`, but may only be used as the type `T` after it has been
used as a reference parameter of type `Core.MaybeUnformed(T)` to a call
other than the type's own destructor. That first call is required to initialize
the object fully.
...
is that in practice these would likely be directly provided by builtin operations.
...
class OwningPtr(T:! type) {
  ...
  // destructor calls from MaybeUnformed(OwningPtr(T)) to do that test themselves.
  fn destroy[ref self: Self]() {
    SomeDeallocationFunction(self.ptr);
  }
}
