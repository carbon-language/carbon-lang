...
any implicit conversions, and so-on. And importantly, this would occur even when
we know that the types of `x` and `y` are exactly `i32` and we might be able to
directly hard coded to a builtin implementation.
