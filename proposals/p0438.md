...
-   Desire to abbreviate "function", although `fn` is clearly shorter than
    `func`.
...
-   Familiarity for developers:
    ...
    -   `fn` is only used by Rust, which by most measures has less adoption than
        ...
        so some C++ developers will be familiar with it that way, if they've
        used Hungarian with function pointers.
