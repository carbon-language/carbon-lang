...
`func` _function name_ `(` _arguments_ `)` _[_ `->` _return type_ _]_ `{` _body_
`}`
...
`fn` and `func` are the favored options. This proposal chooses `func` over `fn`
for three reasons:

-   Length: there's a desire for writing less than "function", so we are
    pursuing an abbreviation.
    -   `func` is 4 characters; `fn` is 2 characters.
-   Consistency:
...
-   Familiarity: both will likely be immediately familiar to many developers,
    but `func` will likely be more familiar as a keyword.
    ...
    -   `fn` is only used by Rust, which by most measures has less adoption than
        ...
        which many C++ developers may be familiar with already.
        -   It's also used for the
            [fn key](https://en.wikipedia.org/wiki/Fn_key), although assumptions
            about the meaning of "n" as "abbreviation for function" versus
            "number" may be initially misleading.
...
-   This proposal borrows portions of trailing return function syntax from C++.
    ...
    -   We prefer trailing return syntax because we expect increased
        readability, and it allows the identifier to be adjacent to `func`.
...
-   Carbon is using `func`, which echoes Go and Swift. This gives familiarity to
    a significant amount of engineers while also offering significant
    abbreviation the longer "function".
