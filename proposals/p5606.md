...
proposal document itself. Furthermore, the rendered proposal document is more
legible than the PR's raw markdown (sometimes dramatically so, such as when the
document contains a table or graph).

Finally, in the event that the reader wants additional information from the PR,
every proposal document has a PR link at the top, so the additional burden on
those readers is a single click. By contrast, the process of navigating from a
