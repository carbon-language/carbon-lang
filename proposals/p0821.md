...
the total number of moves by avoiding moving both first into the parameter and
then out of the parameter. This kind of micro-optimization of moves was seen as
important because some interesting data structures, especially in the face of
...
For immutable indexed access, the suggested
[operator overloading](#operators-overloading-through-implementing-generic-interfaces)
approach of a syntactic rewrite is at least expressive enough. The primary
limitation will be the [return value](#return-values-and-accessors) being
copied. Any lifetime-based approach Carbon develops to remove the copy should
also be applicable to this use case.

Another option is to use a pattern similar to
[user-defined dereferencing](#user-defined-pointer-like-types-and-dereference-operations),
