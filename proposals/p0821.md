...
Here are a list of use cases that this proposal will consider explicitly:
...
parameters which _consume_ their input, as _consuming input_ is the dominant use
...
to gracefully degrade in the absence of move semantics. This also helped reduce
the total number of moves by avoiding moving both first into the parameter
...
A common reason for using mutable references outside of what has already
been described is to represent _non-null pointers_ with enforcement in the
type system. Because
...
(even if not in another thread), the indirection is fundamental to the semantics.
Reading a copy of the pointed to value would be _incorrect_ in these cases
because the copy might be stale. Similarly writes through the pointer should
be reflected in changes to the storage that pointer points to.
...
For immutable indexed access, the suggested [operator overloading](#operators-overloading-through-implementing-generic-interfaces)
approach of a syntactic rewrite is at least expressive enough. The primary
limitation will be the [return value](#return-values-and-accessors) being copied. Any lifetime-based
approach Carbon develops to remove the copy should also be applicable to this
use case.

Another option is to use a pattern similar to [user-defined dereferencing](#user-defined-pointer-like-types-and-dereference-operations),
...
[consuming](#variables-introduce-l-values-with-mutable-storage) input pattern. But it is possible that more will be needed in
...
proposed here or for [classes](/docs/design/classes.md):
