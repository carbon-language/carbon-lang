...
> **TODO:** This rule doesn't fully work. `impl`s can be declared in multiple
> scopes, and unqualified lookup results in different scopes can be different,
> so syntactic matching is insufficient to determine whether `impl`s are
> redeclarations. Also, `impl as` in a class can't be redeclared outside the
> class with this rule, but we can rewrite that to `impl Self as` to transform
> that problem into the scoping problem.
