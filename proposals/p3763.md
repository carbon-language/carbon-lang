...
-   [Alternatives considered](#alternatives-considered)
    ...
    -   [Disallow shadowing in `impl` after use in `api`](#disallow-shadowing-in-impl-after-use-in-api)
...
which is either in the same file or, for a declaration in an `impl` file, in the
`api` file, so we can always perform a precise syntactic redeclaration check if
we persist syntactic information from the `api` file to `impl` files. Some
...
This rule applies only within a single file. For an entity that is declared in
an `api` file and redeclared in an `impl` file, an additional rule is imposed:

-   If an entity is declared as non-`extern` in an `api` file and redeclared in
    an `impl` file of the same library, it is an error for the declaration to
    use any unqualified name that was introduced in the `impl` file.

It is, however, valid for such a declaration to use a name that was introduced
in the `api` file and redeclared in the `impl` file. This ensures that the
declaration has the same unqualified lookup results as a declaration in the
`api` file.

[Additional rules](#extern-declarations) apply to `extern` declarations.
...
class A {
  impl as I where .T = ();
  // Redeclaration of previous declaration.
...
> This was
> [agreed in discussion on 2024-03-11](https://docs.google.com/document/d/1s3mMCupmuSpWOFJGnvjoElcBIe2aoaysTIdyczvKX84/edit?resourcekey=0-G095Wc3sR6pW1hLJbGgE0g&tab=t.0#heading=h.p69b78lovqb7)
...
> **Note:** As part of associating `impl` declarations with a scope, we
> [agreed](https://docs.google.com/document/d/1s3mMCupmuSpWOFJGnvjoElcBIe2aoaysTIdyczvKX84/edit?resourcekey=0-G095Wc3sR6pW1hLJbGgE0g&tab=t.0#heading=h.p69b78lovqb7)
> to also change the `impl forall` syntax to:
>
> ```
> impl [T:! type](T as Interface(T));
> ```
>
> When that change is made, the scope syntax will similarly change to reflect
> the new syntax:
>
> ```
...
### Disallow shadowing in `impl` after use in `api`

A name can be shadowed in an `impl` file after it is used in an `api` file:
...
If this code were all in a single file, the declaration of `class N.A` would be
invalid, because it would change the meaning of the name `A` in the first
declaration of `N.F`. However, in this proposal, we permit the declaration of
`class N.A` in the `impl` file, and instead say that the redeclaration of `N.F`
is invalid because it uses the unqualified name `A` that is local to the `impl`
file -- a declaration that redeclares an entity declared in the `api` file
cannot use names that are local to the `impl` file.

We could use a more restrictive rule here: we could instead say that it is an
error if an `impl` file introduces a name that, if reordered into the `api`
file, would change the meaning of a declaration.

Advantages:

-   More consistent behavior, because the same rule would be used within files
    in a library and across files in a library.
-   Moving code from an `impl` to an `api` file wouldn't introduce new errors
    from name shadowing, and wouldn't risk changing the meaning of code in the
    `api` file.
    -   However, this refactoring can still change the meaning of the code that
        was moved, if it names something that is not declared in the `api` file.
    -   Similarly, moving code from `impl` to `api` can still change the meaning
        of code in other `impl` files.

Disadvantages:

-   Implementations would need to track which names are used in each scope in an
    `api` file in order to provide this enforcement.
-   Coupling the unqualified lookup behavior across files in this way may be
    surprising.
