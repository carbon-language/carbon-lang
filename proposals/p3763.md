...
file. An entity shared by multiple `impl` files must be declared in the `api`
file, typically as a `private` member of the library. If it is not defined in
the `api` file, it must be defined in exactly one `impl` file in that library.
