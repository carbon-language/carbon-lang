...
FIXME: Concern: boilerplate. Concern: no use case for implementing a destructor
outside of the class, so expect all implementations to be `final`. Concern:
making sure correct category for type. Concern: providing default destructor.
Concern: destructor will typically need access to private members. Considered
allowing destructors to be defined externally for classes without private
members.

### Don't distinguish safe and unsafe delete operations

FIXME: This option is more like C++. Would like to try the safer option to see
if it is viable or if it causes problems with interoperation with C++. We expect
that migrated C++ code can fall-back to using `UnsafeDelete` if using `Delete`
is too burdensome.
