...
For the specific case where the developer wants to specialize the destructor
to be trivial under certain circumstances, we could allow the developer to
directly implement `TrivialDestructor`. That implementation would include the
criteria when the trivial destructor should be used. For example, we might
express that `Optional` has a trivial destructor when its argument does by
writing:
