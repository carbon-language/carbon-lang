...
operands of each of the operators? This may be a priori ambiguous, such as in
the expression `a $ b ^ c`: is this `(a $ b) ^ c` or `a $ (b ^ c)`?
...
expressions, but the absence of such parentheses are not enforced in all cases,
even by many linting tools, and forgetting those parentheses can lead to subtle
bugs.

## Proposal

Do not have a total ordering of precedence levels. Instead, define a partial
ordering of precedence levels, and reject due to ambiguity if the meaning of the
program depends on the relative order of two operators that are unordered.
...
With those precedence rules, `a + b * c` would be parsed as `a + (b * c)`,
because `+` has lower precedence than `*`, and `a + b << c` would be an error,
requiring parentheses, because the precedence levels of `+` and `<<` are
unordered.
...
every professional Carbon programmer to remember the precedence, or there should
not be a precedence rule.**
