...
-   **Enum and constexpr variables**: if a macro contains an enum constant or a
    constexpr variable in the replacement list, it will be imported to Carbon as
    an alias to the constant variable.
...
This preserves the type and addressability of the referred variables.
...
-   Importing macros referring to enum and constexpr variables as constants in
    Carbon

    -   _Reason to do this_: This would be a simpler implementation and
        consistent with the other types of object-like macros.
    -   _Reason not to do this_: This won't preserve the type and addressability
        of the referred variables.

-   Support _only_ macros in the global namespace (Cpp.MACRO)
    -   _Reason to do this_: This would support cleaner macros definitions.
    -   _Reason not to do this_: It may be limiting which macros can be
        imported, especially for codebases that can't be easily modified.
