...
repeated matches against the same `_pattern_`, later comparisons may be skipped
by reusing the result from an earlier comparison:
...
type of the scrutinee, and the result is the type of the _identifier_. In either
case, the scrutinee is implicitly converted to the type of the _identifier_.
...
The pattern `auto` is shorthand for `template _:! Type`.
...
fn F(n: i32) {
  ...
  // Equivalent to:
  var w: (template _:! Type) = SomeComplicatedExpression(n);
}
```
...
-   _proper-pattern_ ::= `{` [_field-init_ `,`]\* _proper-field-pattern_ [`,`
    _field-pattern_]\* `}`
-   _proper-pattern_ ::= `{` [_field-pattern_ `,`]+ `...` `}`
