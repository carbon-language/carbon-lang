...
-   A pattern _P_ is _useful_ in the context of a set of patterns _C_ if there exists
    a value that _P_ can match that no pattern in _C_ matches.
...
-   A pattern _P_ is _refutable_ if there are values that it does not match,
    that is, if the pattern `_: auto` is useful in the context of {_P_}. Equivalently,
    the pattern _P_ is _refutable_ if the set of patterns {_P_} is not
    exhaustive.
...
-   When a set of patterns have no ordering or tie-breaker, it is an error for
    ...
    -   For a set of `impl`s that match a given `impl` lookup, argument
        ...
        same type structure are an error unless a `match_first` declaration
        is used to order the `impl`s. (This is a pre-existing rule and is
