...
-   A pattern _P_ is _refutable_ if there are values that it does not match,
    that is, if the pattern `_` is useful in the context of {_P_}. Equivalently,
    the pattern _P_ is _refuatble_ if the set of patterns {_P_} is not
    exhaustive.
  ...
  fn Print[me: Self](stream: Cpp.std.ostream*) {
    match (me) {
      ...
      case .rotate(degrees: f64) => {
        stream->Print("Rotate by {9} degrees.\n", degrees);
      }
    }
  }
}
