...
-   [Alternatives considered](#alternatives-considered)
    ...
    -   [Require declaration and definition to match](#require-declaration-and-definition-to-match)
...
-   **Interoperability with and migration from existing C++ code**
    -   This proposal rejects some constructs that would be valid in C++.
        However, the fact that those constructs are valid in C++ is surprising
        to many, and the constructs that would be idiomatic in C++ are still
...
### Require declaration and definition to match

We could treat the choice of function with `()` return type versus procedure as
being part of the interface rather than being an implementation detail. In
particular, we could disallow any use of a procedure call in a context that
depends on its return value, treating a procedure call as a statement rather
than as an expression that can be used as a subexpression or an operand of an
operator.

This would fully divorce functions and procedures, and remove all special
treatment of `()` in this context, as we would no longer need to specify a
return type for a procedure. In addition, this would mean that adding a return
type to a procedure -- converting it to a function -- would be a non-breaking
change. However, having distinct notions of function versus procedure would be a
surprise for those coming from C++, and we don't have evidence that the
transformation of a procedure into a function is a sufficiently common operation
during code evolution to justify this divergence from programmer expectations.
