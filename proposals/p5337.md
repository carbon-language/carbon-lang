...
We considered three options in discussions in
...
1.  Final impls in the file with the interface are prioritized over the final
    impls in the file with type (when they are different).
2.  Final impls with a `?` for the whole type are prioritized over final impls
    with a concrete root type.
3.  Final impls are prioritized using the reverse of type structure
    prioritization.

The concern with option 1 was that concatenating files that contain overlapping
final impls, such as when creating a reproduction of a bug, would necessitate
`match_first` changes. This option maximized the use of explicit `match_first`
to resolve priority on final `impl` overlap.
