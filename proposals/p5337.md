...
any other overlapping `final impl` defined in the file with the type. If there
is any other overlap between `final impl` definitions, it must be within the
same file and can be resolved using
...
Ties within a priority are resolved using type structure and
`impl_priority`/`match_first` blocks, using the existing rules.

#### An `impl` that can never match is an error

FIXME: configuration constants to disable this, for example for evolution.
