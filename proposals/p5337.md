...
-   Allow `final` on a `match_first` block, and update the rules for `impl`
    lookup.
...
QUESTION: Is it `extend require I` or `extend require impls I`?
...
NOTE: can only be used in the same file.
...
-   If interface `B` has `extend require A`, than any `impl C as B` will require
...
this case. By writing `final` we ensure that `Convert` in `ImplicitAs` always
matches `Convert` in `As`, when they are both defined.
...
FIXME: Do we need to have another proposal for redeclaring an `impl` declared in
`class` scope?
