...
-   [Alternatives considered](#alternatives-considered)
    ...
    -   [Associated constants from a `final match_first`](#associated-constants-from-a-final-match_first)
...
The compiler will only use a known matching final `impl`'s associated constants
if it is the first that _could_ match, as determined by type structure. For
example:
...
final match_first {
  ...
  // since it is the first, so nothing could match
  // ahead of it.
  impl forall [U:! Y] C(U) as L(bool) where .B = i32;

  // Can use the associated constants from this impl
  // only if V can't be `C(U)`, and so it is known
  // that it won't use the previous impl instead.
  impl forall [V:! W] V as L(bool) where .B = bool;

  // Can use the associated constants from this impl,
  // since a query for `T as L(i32)` can't match any
  // of the earlier impls in the `final match_first`.
  impl forall [T:! Z] T as L(i32) where .B = f32;
}

class D {}
impl D as W {}
// D as L(bool) definitely uses the middle `impl forall`
// from the `final match_first` so `(D as L(bool)).B` is
// `bool`.
let x: (D as L(bool)).B = true;

class E(T:! type) {}
impl forall [T:! type] C(E(T)) as W {}
// `C(E(T))` does impl `W`, so it impls `L(bool)`. But
// since we don't know whether `E(T)` impls `Y`, we
// don't know which impl it will use, and so
...
pick the first one listed in the `final match_first` block.
...
### Associated constants from a `final match_first`

We considered a couple of alternative rules for determining when associated
constants were considered to be known, and what value to use.
