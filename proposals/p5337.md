...
-   Types only need to implement the extending interface to also get an
...
First, since we are creating another way that an `interface` can `extend`
...
To allow an `impl` to overlap a final `impl`, we say the final `impl` is
prioritized over a non-final `impl` anytime they overlap. This addresses a
...
sound choice is to prefer the final `impl`. However, if an `impl` can never be
used because it is completely subsumed by a final `impl`, that is an error.

To allow multiple interfaces extending the same interface, we also want to
support a way to declare overlapping final impls. In this case, list the impls
in a `final match_first` block instead of putting `final` on the `impl`
declaration itself. To put the impl generated by an `extend impl as Foo` in an
interface `Bar` in a `match_first` block (or a `final match_first` block), we
give it the name `impl Bar.(as Foo)`.
...
class MyIntIterator { ... }
// Implementing OutputForwardIterator also implements:
...
-   Impls with the same type structure are considered in the order they appear
    in their common `match_first` block.
...
`impl`). In order to detect the subsumption, the query for the final `impl`
...
> _**Note:**_ The question of how to write this when the extending interface is
> parameterized is the subject of
> [leads question #5367: `impl` in `class` redeclaration syntax with parameterization](https://github.com/carbon-language/carbon-lang/issues/5367).
