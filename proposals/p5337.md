...
-   [Details](#details)
    ...
    -   [Final `impl` priority](#final-impl-priority)
        -   [Overlapping `final` impls in different files](#overlapping-final-impls-in-different-files)
        -   [Overlapping `final` impls in the same file](#overlapping-final-impls-in-the-same-file)
...
-   Allow a non-final `impl` to overlap a final `impl` as long as it isn't
    ...
    overlap.
-   Create rules for prioritizing between final impls, to allow them to overlap.
...
support a way to declare overlapping final `impl`s. Each final `impl`
...
When there are final `impl`s that match a query in two different files, the ones
with the interface are given priority.

In the case when there are overlapping final `impl`s declared in the same file,
list the `impl` names in a `final match_first` block instead of putting `final`
on the `impl` declaration itself. To put the `impl` generated by an
`extend impl as Foo` in an interface `Bar` in a `match_first` block (or a
`final match_first` block), we give it the name `impl Bar.(as Foo)`.
...
#### Overlapping `final` impls in different files
...
defining the interface in order to write its final `impl` definition, and so
will be able to see any `final` blanket impls.
...
#### Overlapping `final` impls in the same file
...
-   Second priority: A final `impl C(...) as I(...)` in the file with definition
    of type `C(...)`, when that file does not contain the interface
...
-   Final `impl`s are considered in the order they appear in the
    `final match_first` block, if any, first considering those from the file
    with the interface and those from the file with the type (when different).
...
[overlapping `final` impl rules](#overlapping-final-impls-in-the-same-file), at
most one interface can `final extend impl as` a given interface.
`final match_first` must be used instead of multiple `final impl`s, so that
there is an explicit prioritization between them, as in this example:
...
This question was considered in the
["Overlapping `final` impls" section](#overlapping-final-impls-in-the-same-file).
...
We considered FIXME options in discussions in
...
3.  Final `impl`s are prioritized using the reverse of type structure
    prioritization.
4.  The syntax of final `impl`s associates them with either the interface or the
    root self type. Those associated with the interface are prioritized first,
    and then those associated with the type are prioritized after. Ties are
    resolved by type structure and then `match_first`.
...
The concern with option 4 was no clear choice of syntax to distinguish the two
cases.

In general, options 1-4 were more complex than option 5. This option maximized
the use of explicit `match_first` to resolve priority on final `impl` overlap,
rather than an implicit prioritization. We were also interested in this
property:
...
which options 1-3 did not satisfy.
