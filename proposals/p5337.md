...
support a way to declare overlapping final `impl`s. In this case, list the
`impl` names in a `final match_first` block instead of putting `final` on the
`impl` declaration itself. To put the `impl` generated by an
