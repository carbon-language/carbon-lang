...
support a way to declare overlapping final `impl`s. We do this by listing the names of the
...
named as `impl Bar.(as Foo)`. This name can be used to put such an `impl` into 
a `match_first` block (or a `final match_first` block).
