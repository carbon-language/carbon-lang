...
support a way to declare overlapping final `impl`s. List the names of the
overlapping `impl`s in a `final match_first` block instead of putting `final` on
the `impl` declarations. Note this requires all of the final `impl`s to be in
the same file.

To put the `impl` generated by an `extend impl as Foo` in an interface `Bar` in
a `match_first` block (or a `final match_first` block), we give it the name
`impl Bar.(as Foo)`.
