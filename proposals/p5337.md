...
First, since we are making the `require Self impls I` usage more common in
`interface` and `constraint` scopes, we allow the `Self` to be omitted. So:
...
only type that can be constrained is `Self`:

-   `extend require Self impls I` ✅ allowed
-   `extend require impls I` ✅ allowed, same as `extend require Self impls I`
-   `require T impls I` ❌ forbidden

This echos the similar restriction on `extend impl` in `class` scope.
...
that the definition of the blanket `impl` is generated automatically (and `impl` without
`extend` is not allowed in an interface).
...
Any other `impl` of `As(T)` is going to overlap unless we could
...
negative constraint. So there would be no way to prove the following `impl` of
`As(T)` doesn't overlap the `final impl` of `As(T)`:
...
be used instead of multiple `final impl`s, so that there is an explicit prioritization between
