...
abstractly represent a sequence of outcomes, which can be used to initialize
...
sequence of _named_ outcomes. Consequently, there would be no implicit
conversion from a tuple value to a tuple form, and so `array` could disallow `b`
by requiring its initializer to have a tuple form.
...
lifetime overlap between the outcomes of the function calls that make up the
...
depth-first evaluation. Our top goal for the language is to support
[performance-critical software](/docs/project/goals.md#performance-critical-software),
so this consideration takes priority.
