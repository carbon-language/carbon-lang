...
-   The ability to write Carbon as a "rigorously memory safe programming
    ...
    -   This does not require complete elimination of data races, but does
        require
        [addressing temporal memory safety even in the face of concurrency](#data-races-vs-unsynchronized-temporal-safety).
...
-   A smooth, incremental path for this memory unsafe Carbon code to become more
    ...
    -   Must resonate with customers as _significantly_ more incremental than
        ...
        C++ to Rust.
-   Similar level of runtime spatial safety checks as Rust with the similar
    runtime overhead.
...
However, these improvements aren't free: they come at the cost of added
complexity in the type system itself. Fundamentally, while we will strive to
keep Carbon as simple as we can within its design constraints, we expect Rust to
be a simpler language than Carbon, and for Carbon's improved interop model and
incremental adoption to always come at the cost of complexity. We also don't
expect this complexity to be something that can be fully hidden in
implementation details of standard libraries; some of it will be fundamental and
visible to user code in order to allow the greater degree of flexibility.
...
-   Including constructs parallel to C++ constructs that are not present in
    Rust.
-   More precise modeling using more pointer types, to allow _safe_ mutable
    aliasing.
...
kinds of safe "pointers": shared borrows (`&`), and mutable borrows (`&mut`).
The latter are required to be exclusive -- no other safe "pointers" alias the
mutable object. We expect to have more distinguished types of "pointers" in
Carbon to be able to model more C++ coding patterns safely, including both
shared mutable "pointers", and exclusive mutable pointers -- this is where the
complexity increases. While the exact design of this needs to be carefully
fleshed out, our current idea is to build on the concepts of
...
idioms and concepts. For example, in a container, we expect iterators to use
...
improve its C++ interop, and we plan to develop these with active collaboration
...
safety component to be concrete to do any further evaluation of Carbon, and are
accelerating our work to meet this need.
