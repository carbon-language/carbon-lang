...
other way, such as a named function or an `xor` keyword. Similarly we considered
using a `compl` keyword for the complement operator. The primary motivation was
to avoid spending two precious operator characters on relatively uncommon
