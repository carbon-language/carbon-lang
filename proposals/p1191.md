...
-   `<<` and `<` are both visually similar, analogously to `&` and `&&`.
-   Carbon has moved away from `&&` and other punctuation based _logical_ operators and towards keywords like `and`. Bitwise operators could similaly switch to keywords like `bitand`.
...
-   While some visual similarity exists such as between `<` and `<<`, the contexts these operators are used are sufficiently different to avoid any serious concerns.
