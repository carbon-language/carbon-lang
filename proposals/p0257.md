...
The unformed state can have the same representation as valid and fully formed states for
...
are directly copied into other padding bytes.
...
as `returned` from the function. Again assuming that `MyType` has an unformed
state, the alternative to the above syntax would be:
...
And again we can initialize the return variable to remove the unformed
requirement:
...
-   As above, but require `return <name>;` where `<name>` is constrained to
...
2. Any `return` with a `returned var` in scope must be `return var;` and
   returned the declared `returned var`.
...
fn PointConstructor -> var p: Point = PointConstructorPreObjectPhase() {
...
```
fn SimplerPointConstructor -> var p: Point {
...
fn PointConstructorWithDeclaredReturnVar() -> Point {
  // Code that runs prior to an object existing.
  returned var p: Point = (<x-expression>, <y-expression>);
...
An alternative approach to [declaring `returned` variables](#declared-returned-variable) is to extend the
