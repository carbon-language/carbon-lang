...
FIXME: Support customization of the prefix-`*` dereferencing operator.
...
Then `*p` is rewritten to `p.(Deref.Op)()`.
...
```carbon
class HasMember {
  // Eventually these functions would be
  // together in an overload set.
  fn Get[self: Self]() -> i32 { return self.m; }
  fn GetMut[bound ref self: Self]() -> i32 {
    return self.m;
  }
  var m: i32;
}
