...
-   [Proposal](#proposal)
    ...
    -   [`ref` and `let` returns](#ref-and-let-returns)
-   [Details](#details)
...
Similarly, the `BindToValue` interface is changed to use a `let`/value return,
...
interface BindToValue(T:! type) {
  extend Bind(T);
  fn Op[self: Self](bound x: T) -> let Result;
}
```
...
reliably be used to initialize a variable, particularly in the middle of an object,
...
If requiring `ref` parameters to be `noalias` ends up being too restrictive, we could instead
