...
-   The behavior of a call to a function declared `-> T` is unchanged. It is an
    initializing expression. It will either match `-> var T` or return by copy,
    depending on the initializing representation of `T`.
-   The behavior of `auto` as the return type is unchanged, but now supports an
    optional `ref`, `let` or `var` between the `->` and `auto`. `-> auto` and
    `-> var auto` return an initializing expression, `-> let auto` returns a
    value expression, and `-> ref auto` returns a durable reference expression.
...
var x: T = H();
// x.p == &x at this point, but might not be the case if
// the signature of `G` were `fn G() -> T` (depending
// on the initializing representation of T).
```
