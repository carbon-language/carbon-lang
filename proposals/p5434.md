...
`[`...`]`, see the [indexing design](/docs/design/expressions/indexing.md):
...
With these changes, `lhs[index]` with `lhs` of type `T` and `index` of type `I`
will be rewritten to one of:

-   Before: `*(lhs.(IndirectIndexWith(I).Addr)(index))` -> After:
    `lhs.(IndirectIndexWith(I).Ref)(index)`
-   Before: `*(lhs.(IndexWith(I).Addr)(index))` -> After:
    `lhs.(IndexWith(I).Ref)(index)`
