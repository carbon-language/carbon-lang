...
variables of type `BigInt`. At run time, the kernel will always be a constant or
run-time value (FIXME this is a weird and probably wrong way of saying that),
and the arity will always be a constant. As their name suggests, pack elements
cannot occur on their own, but only as an element of an enclosing pack.
...
elements with the same kernel that do. The _shape_ of a pack is another pack,
which is found by replacing each kernel with `()`. A _pack type_ is the type of
a pack, and can be represented as a pack value whose kernels match types. The
type of a pack is found by replacing each of its kernels with the kernel's type.
Thus, the relationship between packs and pack types is much the same as between
