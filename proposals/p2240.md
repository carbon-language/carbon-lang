...
name binding with a free `[:]` in the type subpattern, so we apply the rule for
name bindings prefixed with `[:]`. That dictates that `vectors` is bound to a
3-tuple, whose Ith element is bound to the value of the Ith iteration's
