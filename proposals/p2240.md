...
rather than some lexical scope. The size of a pack is represented as a sum of
positive integer constants and symbolic variables of type `BigInt`. As their
name suggests, pack elements cannot occur on their own, but only as an element
of an enclosing pack.
...
always possible because a pack element size is always a sum of positive values,
...
(doesn't contain pack expansions), and all types are known constants.
...
-   First N and last M pattern elements are irrefutable for the corresponding
    scrutinee element types
-   The `...,` operand pattern is irrefutable for the pack type of the remaining
    scrutinee elements.
...
The `[:]` pattern is irrefutable for a given scrutinee pack type if the operand
pattern is irrefutable for the corresponding tuple type.
