...
FIXME check for consistency with the rest of the doc
...
values that all match a single pattern. During symbolic evaluation, the pattern
and the arity of the sequence are represented as
...
rather than some lexical scope. The arity of a pack is represented as a sum of
positive integer constants and symbolic variables of type `BigInt`. At run time,
the pattern will always be a constant or run-time value (FIXME this is a weird
and probably wrong way of saying that), and the arity will always be a constant.
As their name suggests, pack elements cannot occur on their own, but only as an
element of an enclosing pack.
...
symbolic value consisting of a single variable name, or a constant 1. This is
always possible because a pack element arity is always a sum of positive values,
so an element that doesn't fit this criterion can be split into a sequence of
elements with the same pattern that do. The _shape_ of a pack is another pack,
which is found by replacing each element's pattern with `()`. A _pack type_ is
the type of a pack, and can be represented as a pack value whose element
patterns match types. The type of a pack is found by replacing each of its
element's patterns with the type of that element's pattern. Thus, the
relationship between packs and pack types is much the same as between tuples and
tuple types.
...
-   Otherwise, we evaluate the element expression to produce a value `V`, and
    append a pack element with pattern `V` and arity 1 to the result tuple.
...
These are run-time semantics, so any pack elements in the scrutinee value have
arity 1, and all types are known constants.
...
operand of the `...,` pattern is matched with the sub-pack consisting of any
remaining scrutinee elements.

_Name binding pattern:_

If the name binding pattern has a pack type, it is bound to a pack consisting of
...
A `[:]` pattern consists of a `[:]` token and a single operand pattern. The
scrutinee must be a pack, and the `[:]` pattern matches if the operand pattern
matches a tuple consisting of the elements of the scrutinee pack.
...
element patterns are `()`. In other words, statements have types, but they only
...
have arity 1, and "variadic" if they have indeterminate arity (these are the
only possibilities, because packs are normalized).
...
argument. If E's rewritten arity is sufficiently large, and all earlier variadic
...
that if the variadic parameter type's pattern involves a deduced value that is
used in more than one place (as `Ts` is in the earlier example of this problem),
there cannot be any leading or trailing variadic symbolic arguments (in the
sense defined in the previous section). This ensures that each symbolic argument
can only match one parameter, and so type deduction deterministically produces a
single result.
