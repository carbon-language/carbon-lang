...
-   [Alternatives considered](#alternatives-considered)
    -   [Variadic members](#variadic-members)
...
> **TODO:** change "pack expansion" to "expansion", and change "expansion
> argument" to "expansion parameter".
...
A "pack expansion" is a syntactic unit beginning with `...`, which is a kind of
compile-time loop over sequences called "packs". The packs that an expansion
iterates over are generated by syntactic units called "expansion arguments".
Expansion arguments can be formed from tuples with the `expand` operator, or
from a "variadic binding", which is marked with the `each` keyword at the point
of declaration and the point of use.
...
However, this raises some novel design issues. In particular, variadic bindings
...
variadic members by using members with tuple or array types instead.
Consequently, this feature is deferred to future work.
...
This employs three separate features not in the primary proposal:

-   In cases where the declared type of the variadic binding does not vary
    across iterations (like `ElementType`), we can re-express it as an array
    binding if we have support for array expansion arguments (discussed above).
-   In cases where the declared type of the binding does vary, that fact alone
    implies that the binding is variadic, so we can effectively infer the
...
-   The implicit expansion of pack-type bindings hurts readability. For example,
    ...
    arguments, not just one. This problem is especially acute in cases where a
...
-   We have to forbid template-dependent names from having pack types (see
    ...
    because the possibility that an expression might be an expansion argument in
...
pack expansions within expansion arguments. This does create some risk of
confusion about which pack expansion an expansion argument belongs to, but
probably much less than if we allowed unrestricted nesting.
...
for tuple element lists. Otherwise, `...*expand foo` would be ambiguous between
...
function signatures can have bindings below top-level, which is currently
undecided.
...
nontrivial complexity to the design. In particular, when an expansion argument
is lexically within two or more pack expansions, we need a rule for determining
which pack expansion it's an argument to, in a way that is unsurprising and
...
However, prefixe syntaxes are usually easier to parse (particularly for humans),
...
noisy than `...`, and they may also be more self-explanatory. However, it does
have some substantial drawbacks.
