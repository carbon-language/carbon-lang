...
computation adds some major challenges here as well.

However, if tuples could ever be library types, they would certainly need to be
defined variadically, so in that sense this proposal may actually support that
principle. In addition, the fact that pack types are built-in is arguably
mitigated by the fact that pack types cannot be explicitly named in Carbon code.
