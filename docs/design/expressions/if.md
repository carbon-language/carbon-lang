...
assumed to be `T`, even in contexts where `T` involves a generic parameter or
symbolic binding and so the result would normally be an unknown type whose facet
type is `type`.
