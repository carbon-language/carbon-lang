...
class Args {
 ...
 public:
  ...
  // to `out`.
  static auto Parse(llvm::ArrayRef<llvm::StringRef> unparsed_args,
 ...
 private:
  ...
  friend auto operator<<(llvm::raw_ostream& output, ParseResult result)
      -> llvm::raw_ostream& {
    switch (result) {
  ...
  friend auto operator<<(llvm::raw_ostream& output, ArgKind kind)
      -> llvm::raw_ostream& {
    switch (kind) {
      case ArgKind::Flag:
  ...
  friend auto operator<<(llvm::raw_ostream& output, CommandKind kind)
      -> llvm::raw_ostream& {
    switch (kind) {
...
template <typename T>
Args::OneOfArgBuilder::OneOfValueT<T>::OneOfValueT(llvm::StringRef str, T value)
    : str(str), value(std::move(value)) {}

template <typename T>
auto Args::OneOfArgBuilder::OneOfValue(llvm::StringRef str, T value)
    -> OneOfValueT<T> {
  return OneOfValueT<T>(str, value);
}

template <typename T, typename U, size_t N>
void Args::OneOfArgBuilder::SetOneOf(const OneOfValueT<U> (&values)[N],
                                     T* result) {
...
template <typename T, typename U, size_t N>
void Args::OneOfArgBuilder::AppendOneOf(const OneOfValueT<U> (&values)[N],
                                        llvm::SmallVectorImpl<T>* sequence) {
  static_assert(N > 0, "Must include at least one value.");
  arg_.is_append = true;
  OneOfImpl(
...
// function objects.
template <typename U, size_t N, typename MatchT, size_t... Indices>
void Args::OneOfArgBuilder::OneOfImpl(
