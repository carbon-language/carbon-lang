...
// Adds support for printing a class to llvm::raw_ostream for normal execution,
// and std::ostream primarily for GoogleTest.
#define CARBON_PRINTABLE(ClassT)                                               \
  /* Provides simple printing for debuggers. */                                \
  LLVM_DUMP_METHOD void Dump() const { Print(llvm::errs()); }                  \
  /* Supports printing to llvm::raw_ostream. */                                \
  friend auto operator<<(llvm::raw_ostream& out, const ClassT& obj)            \
      ->llvm::raw_ostream& {                                                   \
    obj.Print(out);                                                            \
    return out;                                                                \
  }                                                                            \
  /* Supports printing to std::ostream. */                                     \
  friend auto operator<<(std::ostream& out, const ClassT& obj)                 \
      ->std::ostream& {                                                        \
   ...
   * APIs.                                                                     \
   */                                                                          \
  friend auto PrintTo(ClassT* p, std::ostream* out)->void {                    \
