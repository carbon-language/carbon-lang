...
static inline DIR* opendir(const char* name) {
  ...
  if (!wname.empty() && wname.back() == 0) wname.pop_back();
  ...
  // Open fd for dirfd()
  dir->fd = -1;
  HANDLE h = CreateFileW(wname.c_str(), GENERIC_READ,
...
static inline DIR* opendir(const wchar_t* wname_in) {
...
static inline dirent* readdir(DIR* dir) {
  if (!dir || !dir->has_next) return nullptr;
...
static inline DIR* fdopendir(int fd) {
  ...
  if (h == INVALID_HANDLE_VALUE) { errno = EBADF; return nullptr; }
  
  wchar_t dir_buf[32768];
  DWORD len = GetFinalPathNameByHandleW(h, dir_buf, 32768, FILE_NAME_NORMALIZED);
  if (len == 0) { errno = EBADF; return nullptr; }
  
  std::wstring wpath(dir_buf, len);
  // Strip \?\ prefix
  if (wpath.size() >= 4 && wpath[0]==L'\\' && wpath[1]==L'\\' && wpath[2]==L'?' && wpath[3]==L'\\')
