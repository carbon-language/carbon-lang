...
// `extra_chars` allows additional characters, like `/` for library paths.
static auto IdentifierToCarbon(std::string_view s, llvm::raw_ostream& out,
                               std::string_view extra_chars = "") -> void {
  if (s.empty()) {
    out << "EmptyIdentifier";
  } else {
    auto is_extra_char = [&extra_chars](char c) {
      return extra_chars.find(c) != std::string::npos;
    };
    for (size_t i = 0; i < s.size(); ++i) {
      const char c = s[i];
      if (i == 0) {
        if (llvm::isAlpha(c) || c == '_' || is_extra_char(c)) {
          out << c;
        } else {
          out << 'x' << llvm::toHex(c);
        }
      } else {
        if (llvm::isAlnum(c) || c == '_' || is_extra_char(c)) {
          out << c;
        } else {
          out << 'x' << llvm::toHex(c);
        }
...
static auto LibraryNameToCarbon(const Fuzzing::LibraryName& library,
  ...
  if (library.has_path()) {
    out << " library \"";
    IdentifierToCarbon(library.path(), out, /*extra_chars=*/"/");
    out << "\"";
  }
}
