...
Although no aspect of Carbon will ever be "final", some parts of Carbon's design
...
answert the underlying questions and build confidence in the design. However,
...
especially "experimental", consider making it a link to a tracking bug, so that
future readers can see how the issue was resolved.
...
the proposal is accepted. Like "FIXME", it should never appear in the Carbon
repository itself, except in places like the previous sentence. For example:
...
"Hypothetical" is similar to "placeholder", but does not take a position on
whether anything like the feature in question will exist in Carbon. For example:

> This section discusses how our proposal would interact with a **hypothetical**
> user-mode threading feature.
...
> The decision to use prefix notation for all arithmetic is
> [**experimental**](https://forums.carbon-lang.dev/t/dummy-thread), and may
> need to be revisited if user experience shows that it's unworkable.
