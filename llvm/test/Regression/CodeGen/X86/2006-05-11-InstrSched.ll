...
cond_true:		; preds = %cond_true, %entry
	...
	%k.0.0 = bitcast uint %tmp.10 to int		; <int> [#uses=2]
	%tmp31 = add int %k.0.0, -1		        ; <int> [#uses=4]
	%tmp32 = getelementptr int* %mpp, int %tmp31	; <int*> [#uses=1]
	%tmp34 = bitcast int* %tmp32 to sbyte*	        ; <sbyte*> [#uses=1]
	...
	%tmp42 = bitcast int* %tmp42 to <4 x int>*	; <<4 x int>*> [#uses=1]
	%tmp46 = load <4 x int>* %tmp42		        ; <<4 x int>> [#uses=1]
	%tmp54 = bitcast <16 x sbyte> %tmp to <4 x int>	; <<4 x int>> [#uses=1]
	%tmp55 = add <4 x int> %tmp54, %tmp46		; <<4 x int>> [#uses=2]
	%tmp55 = bitcast <4 x int> %tmp55 to <2 x long>	; <<2 x long>> [#uses=1]
	%tmp62 = getelementptr int* %ip, int %tmp31	; <int*> [#uses=1]
	%tmp65 = bitcast int* %tmp62 to sbyte*		; <sbyte*> [#uses=1]
	...
	%tmp73 = bitcast int* %tmp73 to <4 x int>*	; <<4 x int>*> [#uses=1]
	%tmp77 = load <4 x int>* %tmp73		        ; <<4 x int>> [#uses=1]
	%tmp87 = bitcast <16 x sbyte> %tmp66 to <4 x int>	; <<4 x int>> [#uses=1]
	%tmp88 = add <4 x int> %tmp87, %tmp77		; <<4 x int>> [#uses=2]
	%tmp88 = bitcast <4 x int> %tmp88 to <2 x long>	; <<2 x long>> [#uses=1]
	%tmp99 = tail call <4 x int> %llvm.x86.sse2.pcmpgt.d( <4 x int> %tmp88, <4 x int> %tmp55 )	; <<4 x int>> [#uses=1]
	%tmp99 = bitcast <4 x int> %tmp99 to <2 x long>	; <<2 x long>> [#uses=2]
	...
	%tmp137 = bitcast int* %tmp137 to <2 x long>*	; <<2 x long>*> [#uses=1]
