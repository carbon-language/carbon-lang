...
template <typename TargetVecT, typename ValVecT, typename V>
void addDiffForTargSlice(V Val, Target Targ, DiffOutput &Diff,
  ...
  if (TargetVector != Diff.Values.end()) {
    ValVecT NewValVec(Order, Val);
    cast<TargetVecT>(TargetVector->get())->TargValues.push_back(NewValVec);
  } else {
    auto NewTargetVec = std::make_unique<TargetVecT>(Targ);
    ValVecT NewValVec(Order, Val);
    NewTargetVec->TargValues.push_back(NewValVec);
...
void printVecVal<DiffSymVec>(std::string Indent, const DiffOutput &Attr,
  ...
  if (Attr.Values.empty())
    return;
  OS << Indent << Attr.Name << "\n";
  for (auto &RawSymVec : Attr.Values)
    if (DiffSymVec *SymVec = dyn_cast<DiffSymVec>(RawSymVec.get())) {
