...
// CHECK: define dso_local <2 x i64> @testXMMout(<2 x i64>* %p) #0
...
// CHECK: define dso_local <4 x i64> @testYMMout(<4 x i64>* %p) #1
...
// CHECK: define dso_local <8 x i64> @testZMMout(<8 x i64>* %p) #2
...
// CHECK: define dso_local void @testXMMin(<2 x i64> %xmm0, <2 x i64>* %p) #0
void testXMMin(__m128i xmm0, __m128i *p) {
  __asm__("vmovdqu %0, %1" : : "v"(xmm0), "m"(*(__m128i*)p));
}

// CHECK: define dso_local void @testYMMin(<4 x i64> %ymm0, <4 x i64>* %p) #1
void testYMMin(__m256i ymm0, __m256i *p) {
  __asm__("vmovdqu %0, %1" : : "v"(ymm0), "m"(*(__m256i*)p));
}

// CHECK: define dso_local void @testZMMin(<8 x i64> %zmm0, <8 x i64>* %p) #2
