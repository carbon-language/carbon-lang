...
int test_load_m() {
  ...
  asm("l %0, %1" : "=r" (i) : "m" (gi));
  return i;
// CHECK-LABEL: define dso_local signext i32 @test_load_m()
...
int test_load_Q() {
  ...
  asm("l %0, %1" : "=r" (i) : "Q" (gi));
  return i;
// CHECK-LABEL: define dso_local signext i32 @test_load_Q()
...
int test_load_R() {
  ...
  asm("l %0, %1" : "=r" (i) : "R" (gi));
  return i;
// CHECK-LABEL: define dso_local signext i32 @test_load_R()
...
int test_load_S() {
  ...
  asm("l %0, %1" : "=r" (i) : "S" (gi));
  return i;
// CHECK-LABEL: define dso_local signext i32 @test_load_S()
...
unsigned int test_dJa(unsigned int i, unsigned int j) {
  ...
  return i;
// CHECK-LABEL: define dso_local zeroext i32 @test_dJa(i32 zeroext %i, i32 zeroext %j)
...
unsigned long test_rK(unsigned long i) {
  ...
  return i;
// CHECK-LABEL: define dso_local i64 @test_rK(i64 %i)
...
unsigned long test_rL(unsigned long i) {
  ...
  return i;
// CHECK-LABEL: define dso_local i64 @test_rL(i64 %i)
// CHECK: call i64 asm "sllg $0, $1, $2", "=r,r,L"(i64 %i, i32 500000)
}

void test_M() {
  asm volatile("#FOO %0" :: "M"(0x7fffffff));
// CHECK-LABEL: define dso_local void @test_M()
...
float test_f32(float f, float g) {
  ...
  return f;
// CHECK-LABEL: define dso_local float @test_f32(float %f, float %g)
...
double test_f64(double f, double g) {
  ...
  return f;
// CHECK-LABEL: define dso_local double @test_f64(double %f, double %g)
...
long double test_f128(long double f, long double g) {
  ...
  return f;
// CHECK: define dso_local void @test_f128(fp128* noalias nocapture sret [[DEST:%.*]], fp128* nocapture readonly %0, fp128* nocapture readonly %1)
