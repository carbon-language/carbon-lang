...
// CHECK-LABEL: define dso_local signext i16 @f1()
short f1(void) {
  return 0;
}
...
// CHECK-LABEL: define dso_local double @f4()
double f4(void) {
  return 0;
}

// CHECK-LABEL: define dso_local x86_fp80 @f5()
long double f5(void) {
  return 0;
}

// CHECK-LABEL: define dso_local void @f6(i8 signext %a0, i16 signext %a1, i32 %a2, i64 %a3, i8* %a4)
void f6(char a0, short a1, int a2, long long a3, void *a4) {
}

// CHECK-LABEL: define dso_local void @f7(i32 %a0)
...
//
// CHECK-LABEL: define dso_local void @f8_1(%union.u8* noalias sret %agg.result)
// CHECK-LABEL: define dso_local void @f8_2(%union.u8* byval(%union.u8) align 16 %a0)
union u8 {
  long double a;
  int b;
};
...
// CHECK-LABEL: define dso_local void @f10(i64 %a0.coerce)
struct s10 { int a; int b; int : 0; };
void f10(struct s10 a0) {}

// CHECK-LABEL: define dso_local void @f11(%union.anon* noalias sret %agg.result)
union { long double a; float b; } f11() { while (1) {} }

// CHECK-LABEL: define dso_local i32 @f12_0()
// CHECK-LABEL: define dso_local void @f12_1(i32 %a0.coerce)
...
// registers.
// CHECK: define dso_local void @f13(%struct.s13_0* noalias sret %agg.result, i32 %a, i32 %b, i32 %c, i32 %d, {{.*}}* byval({{.*}}) align 8 %e, i32 %f)
...
// rdar://8135035
// CHECK-LABEL: define dso_local void @f18(i32 %a, i32 %f18_arg1.coerce) 
struct f18_s0 { int f0; };
void f18(int a, struct f18_s0 f18_arg1) { while (1) {} }

// Check byval alignment.

// CHECK-LABEL: define dso_local void @f19(%struct.s19* byval(%struct.s19) align 16 %x)
struct s19 {
  long double a;
};
void f19(struct s19 x) {}

// CHECK-LABEL: define dso_local void @f20(%struct.s20* byval(%struct.s20) align 32 %x)
...
void f23(int A, struct f23S B) {
  // CHECK-LABEL: define dso_local void @f23(i32 %A, i64 %B.coerce0, i32 %B.coerce1)
}
...
struct f23S f24(struct f23S *X, struct f24s *P2) {
  ...
  // CHECK: define dso_local { i64, i32 } @f24(%struct.f23S* %X, %struct.f24s* %P2)
}

// rdar://8248065
typedef float v4f32 __attribute__((__vector_size__(16)));
v4f32 f25(v4f32 X) {
  // CHECK-LABEL: define dso_local <4 x float> @f25(<4 x float> %X)
  // CHECK-NOT: alloca
...
struct foo26 f26(struct foo26 *P) {
  // CHECK: define dso_local { i32*, float* } @f26(%struct.foo26* %P)
  return *P;
}
...
struct v4f32wrapper f27(struct v4f32wrapper X) {
  // CHECK-LABEL: define dso_local <4 x float> @f27(<4 x float> %X.coerce)
  return X;
}
...
struct v8f32wrapper f27a(struct v8f32wrapper X) {
  // AVX-LABEL: define dso_local <8 x float> @f27a(<8 x float> %X.coerce)
  return X;
}
...
struct v8f32wrapper_wrapper f27b(struct v8f32wrapper_wrapper X) {
  // AVX-LABEL: define dso_local <8 x float> @f27b(<8 x float> %X.coerce)
  return X;
}
...
void f28(struct f28c C) {
  // CHECK-LABEL: define dso_local void @f28(double %C.coerce0, i32 %C.coerce1)
}
...
void f29a(struct f29a A) {
  // CHECK-LABEL: define dso_local void @f29a(double %A.coerce0, i32 %A.coerce1)
}

// rdar://8249586
struct S0 { char f0[8]; char f2; char f3; char f4; };
void f30(struct S0 p_4) {
  // CHECK-LABEL: define dso_local void @f30(i64 %p_4.coerce0, i24 %p_4.coerce1)
}
...
float f31(struct f31foo X) {
  // CHECK-LABEL: define dso_local float @f31(<2 x float> %X.coerce0, float %X.coerce1)
  return X.c;
}

_Complex float f32(_Complex float A, _Complex float B) {
  // rdar://6379669
  // CHECK-LABEL: define dso_local <2 x float> @f32(<2 x float> %A.coerce, <2 x float> %B.coerce)
  return A+B;
}
...
// rdar://8359248
// CHECK-LABEL: define dso_local double @f34(double %arg.coerce)
v1i64 f34(v1i64 arg) { return arg; }


// rdar://8358475
// CHECK-LABEL: define dso_local double @f35(double %arg.coerce)
...
// CHECK-LABEL: define dso_local double @f36(double %arg.coerce)
...
// CHECK-LABEL: define dso_local i32 @f44
// CHECK: ptrtoint
...
// AVX-LABEL: define dso_local i32 @foo(<3 x i64> %X)
...
// AVX-LABEL: define dso_local void @test53
// AVX-NOT: br i1
// AVX: ret void
...
//
// AVX512-LABEL: define dso_local void @f63
// AVX512-NOT: br i1
// AVX512: ret void
