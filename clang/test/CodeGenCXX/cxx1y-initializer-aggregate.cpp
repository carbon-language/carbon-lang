...
// CHECK: @a = dso_local global {{.*}} zeroinitializer

// @b has a constant initializer
// CHECK: @[[STR_B:.*]] = {{.*}} [8 x i8] c"bazquux\00"
// CHECK: @b = dso_local global {{.*}} i32 4, {{.*}} @[[STR_B]], {{.*}} i8 117, i32 42, {{.*}} i8 9

B x;
B y {};
B z { 1 };
// CHECK: @z = dso_local global {{.*}} { i32 1 }
...
C n{};
// CHECK: @n = dso_local global %union.C { %struct.anon { i64 -1 } }, align 8
