...
namespace test0 {
  ...
  // CHECK:    define dso_local [[A:%.*]]* @_ZN5test04testEs(i16 signext
...
namespace test1 {
  ...
  // CHECK:    define dso_local [100 x [[A:%.*]]]* @_ZN5test14testEs(i16 signext
  // CHECK:      [[N:%.*]] = sext i16 {{%.*}} to i32
...
namespace test2 {
  ...
  // CHECK:    define dso_local [100 x [[A:%.*]]]* @_ZN5test24testEs(i16 signext
  // CHECK:      [[N:%.*]] = sext i16 {{%.*}} to i32
  // CHECK-NEXT: [[T0:%.*]] = call { i32, i1 } @llvm.umul.with.overflow.i32(i32 [[N]], i32 400)
  // CHECK-NEXT: [[T1:%.*]] = extractvalue { i32, i1 } [[T0]], 1
  // CHECK-NEXT: [[T2:%.*]] = extractvalue { i32, i1 } [[T0]], 0
...
namespace test4 {
  ...
  // CHECK:    define dso_local [[A:%.*]]* @_ZN5test44testEs(i16 signext
  // CHECK:      [[N:%.*]] = sext i16 {{%.*}} to i32
...
namespace test5 {
  ...
  // CHECK:    define dso_local [[A:%.*]]* @_ZN5test54testEi(i32
...
namespace test6 {
  ...
  // CHECK:    define dso_local [[A:%.*]]* @_ZN5test64testEt(i16 zeroext
...
namespace test7 {
  ...
  // CHECK:    define dso_local [100 x [[A:%.*]]]* @_ZN5test74testEt(i16 zeroext
  // CHECK:      [[N:%.*]] = zext i16 {{%.*}} to i32
  // CHECK-NEXT: [[T0:%.*]] = call { i32, i1 } @llvm.umul.with.overflow.i32(i32 [[N]], i32 400)
  // CHECK-NEXT: [[T1:%.*]] = extractvalue { i32, i1 } [[T0]], 1
  // CHECK-NEXT: [[T2:%.*]] = extractvalue { i32, i1 } [[T0]], 0
...
namespace test8 {
  ...
  // CHECK:    define dso_local [[A:%.*]]* @_ZN5test84testEx(i64
...
namespace test9 {
  ...
  // CHECK:    define dso_local [[A:%.*]]* @_ZN5test94testEy(i64
  // CHECK:      [[N:%.*]] = load i64, i64*
  // CHECK-NEXT: [[T1:%.*]] = trunc i64 [[N]] to i32
  // CHECK-NEXT: [[T2:%.*]] = call { i32, i1 } @llvm.umul.with.overflow.i32(i32 [[T1]], i32 4)
  // CHECK-NEXT: [[T3:%.*]] = extractvalue { i32, i1 } [[T2]], 1
