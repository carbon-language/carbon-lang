...
namespace ValueInitArrayOfMemPtr {
  ...
  // CHECK-LABEL: define dso_local void @_ZN22ValueInitArrayOfMemPtr1fEi
  void f(int n) {
    Agg1 a = { n };
    // CHECK: store i32 -1,
  ...
  // CHECK-LABEL: define dso_local void @_ZN22ValueInitArrayOfMemPtr1gEMNS_1SEi
  void g(p ptr) {
    // CHECK: store i32 -1,
    f(a{ptr});
  }
...
namespace array_dtor {
  ...
  void f(const T &);
  void f(T *);
  // CHECK-LABEL: define dso_local void @_ZN10array_dtor1gEv(
  void g() {
  ...
  // CHECK-LABEL: define dso_local void @_ZN10array_dtor1hEv(
  void h() {
    // CHECK: %[[ARRAY:.*]] = alloca [3 x
    // CHECK: br
  ...
  // CHECK-LABEL: define dso_local void @_ZN10array_dtor1iEv(
  void i() {
    // CHECK: %[[ARRAY:.*]] = alloca [3 x
    // CHECK: br
