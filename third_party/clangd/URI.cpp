...
class FileSystemScheme : public URIScheme {
 public:
  llvm::Expected<std::string> getAbsolutePath(
  ...
  llvm::Expected<URI> uriFromAbsolutePath(
      llvm::StringRef AbsolutePath) const override {
    std::string Body;
...
llvm::Expected<std::unique_ptr<URIScheme>> findSchemeByName(
